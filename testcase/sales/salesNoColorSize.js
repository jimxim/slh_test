...
//    run("【销售开单－开单】使用上次成交价", "test170107");//
    ...
    // run("【销售开单】单据打印后不允许修改--都不允许修改", "test170238");
...
function test170173() {
    ...
    var ret = isAnd(isEqual("李四", a), isEqual(getToday(""), b), isEqual("1316",
            c));
...
function test170189() {
    ...
    tapButton(window, "核销");
    tapButton(getScrollView(1), "兑换记录");
...
function test170192() {
    // 1开单员005，开单员销售价允许改高，不允许改低；2、开启销售开单修改单价－都可修改
    tapMenu("销售开单", "开  单+");
    var json = { "客户" : "ls", "店员" : "000",
        "明细" : [ { "货品" : "3035", "数量" : "1" } ], "onlytest" : "yes" };
    editSalesBillNoColorSize(json);
