...
function test170018() {
    ...
    var qr = getQR(getScrollView(-1), TITLE_SEQ, 21);
    var totalPageNo = qr.totalPageNo;
    for (var j = 1; j <= totalPageNo; j++) {
        for (var i = 0; i < qr.curPageTotal; i++) {
            sum1 += Number(qr.data[i]["现金"]);
...
function test170028_170036_170487() {
    ...
    // idx = 6;
    // }
    // var f6 = new TField("备注", TF_SC, idx, "退货");
    // var fields = [ f6 ];
    // setTFieldsValue(getScrollView(-1), fields);
...
function test170267() {
    ...
    var qr = getQRDet();
    var ret = isAnd(isEqual(0, qr.data[0]["数量"]), isEqual(0, qr.data[1]["数量"]),
            isEqual(0, qr.data[2]["数量"]), isEqual(0, qr.data[3]["数量"]),
            isEqual(0, qr.data[4]["数量"]));
    tapReturn();
...
function test170268() {
    ...
    tapMenu("采购入库", "新增入库+");
    var json = { "客户" : "vell", "明细" : [ { "货品" : s, "数量" : 50 } ], "现金" : 5000 };
    editSalesBillNoColorSize(json);
...
function test170273() {
    ...
    var ret = isAnd(isEqual("1.5", qr.data[0]["折扣"]), isEqual("1.3",
            qr.data[1]["折扣"]));// , isEqual(money, getTextFieldValue(window,
    // 11))
    saveAndAlertOk();
    tapPrompt();
    tapReturn();
...
function test170305_1() {
    ...
    var je = getTextFieldValue(window, 11);
    var ys = getTextFieldValue(window, 3);
    var ss = getTextFieldValue(window, 4);
...
function test170313() {
    ...
    keys = [ "code" ];
    fields = salesQueryParticularFields(keys);
    changeTFieldValue(fields["code"], "00000");
    setTFieldsValue(window, fields);
    tapButton(window, QUERY);
    qr = getQR();
...
function test170340() {
    ...
    var fields = salesCustomerSupplyFields(keys);
    query(fields);
    var qr = getQR(getScrollView(-1), TITLE_SEQ, 10);
...
function test170361_1_170648() {
    tapMenu("采购入库", "新增入库+");
    var json = { "客户" : "Rt", "明细" : [ { "货品" : "k200", "数量" : 20 } ] };
    editSalesBillNoColorSize(json);
    ...
    var ret = isAnd(isEqual("Rt", qr.data[0]["厂商"]), isEqual(20, sub(xs1, xs)),
            isEqual(8, sub(ts1, ts)), isEqual(12, sub(sxs1, sxs)), isAqualNum(
...
function test170419() {
    ...
    tapFirstText();
    var f9 = new TField("货品", TF_AC, idx + 4, "3035");
    var f14 = new TField("数量", TF, idx + 10, r);
    var fields = [ f9, f14 ];
    setTFieldsValue(getScrollView(-1), fields);
    saveAndAlertOk();
    tapPrompt();
...
function test170431() {
    ...
    tapFirstText();
    var remitTFindex = getEditSalesTFindex2("客户", "汇款");
    var advanceTFindex = getEditSalesTFindex2("客户", "预");
    var money = getTextFieldValue(window, advanceTFindex);
...
function test170433() {
    ...
    query();
    var qr = getQR();
    var ret = isAnd(isEqual(qr.data[0]["订货额"], qr1["总计"]), isEqual(add(xj, sk,
            hk, ds), qr.data[0]["已付"]), isEqual(qr.data[0]["未付"], sub(
    ...
    xj = getTextFieldValue(window, cashTFindex);
    sk = getTextFieldValue(window, cardTFindex);
    hk = getTextFieldValue(window, remitTFindex);
    ds = getTextFieldValue(window, cardTFindex + 1);
    saveAndAlertOk();
    tapPrompt();
    ...
    query();
    qr = getQR();
    var ret1 = isAnd(isEqual(qr.data[0]["订货额"], money1), isEqual(add(xj, sk,
            hk, ds, money), qr.data[0]["已付"]), isEqual(qr.data[0]["未付"], sub(
            qr.data[0]["订货额"], qr.data[0]["已付"])));
...
function test170434() {
    ...
    var remitTFindex = getEditSalesTFindex2("客户", "汇款");
    var money = getTextFieldValue(window, cashTFindex + 3);
    var xj = getTextFieldValue(window, cashTFindex);
    var sk = getTextFieldValue(window, cardTFindex);
    var hk = getTextFieldValue(window, remitTFindex);
...
function test170435() {
    ...
    query();
    var qr = getQR();
    var ret = isAnd(isEqual(qr.data[0]["订货额"], qr1["总计"]), isEqual(money,
...
function test170454() {
    ...
    tapMenu("销售开单", "开  单+");
    var json = { "客户" : "ls", "明细" : [ { "货品" : r, "数量" : 10 } ], "不返回" : "yes" };
    editSalesBillNoColorSize(json);
...
function test170482() {
    ...
    tapFirstText();
    var f10 = new TField("日期", TF_DT, 10, getDay(-2));
    var fields = [ f10 ];
    setTFieldsValue(window, fields);
...
function test170569() {
    ...
    var sf = getTextFieldValue(window, cardTFindex + 1);
...
function test170634() {
    ...
    var json = { "客户" : "lt", "明细" : [ { "货品" : "k300", "数量" : 1 } ],
        "配货" : "004", "现金" : 100, "刷卡" : [ 1000 ], "onlytest" : "yes" };
    editSalesBillNoColorSize(json);
    var qr = getQRDet();
    var totalMoney = qr.data[0]["小计"];
...
function test170669() {
    ...
    var f5 = new TField("数量", TF, idx, "20");
    var fields = [ f5 ];
    setTFieldsValue(getScrollView(-1), fields);
    saveAndAlertOk();
    tapPrompt();
    ...
    tapFirstText();
    var qr = getQRDet();
    var ret = isAnd(isEqual(0, qr.data[0]["数量"]), !isEqual(-10,
            qr.data[0]["数量"]));
    tapReturn();
...
function test170682() {
    ...
    ret = isAnd(ret, isIn(qr.data[0]["货品"], "3035"), isEqual(20,
            ...
            isEqual("仓库店", getTextFieldValue(window, 6)));
    tapReturn();
    ...
    var ret4 = isAnd(isIn(qr.data[0]["货品"], "3035"), isEqual(20,
            qr.data[0]["订货数"]), isEqual(0, qr.data[0]["已发"]), isEqual(20,
            qr.data[0]["数量"]), isEqual("李四", getTextFieldValue(window, 0)));
    ...
    var ret3 = isAnd(isIn(qr.data[0]["货品"], "3035"), isEqual(10,
            ...
            isEqual("李四", getTextFieldValue(window, 0)));
    tapReturn();
