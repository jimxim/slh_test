 ...
 * @returns {QResult}
 */
function getQRverify() {
    // 标题
    var firstTitle = "门店", lastTitle = "备注", firstTitleIndex = 0, lastTitleIndex = 0, titleTotal = 0;
    var texts = getScrollView(-1);
    for (var i = 0; i < texts.length; i++) {
        if (firstTitle == texts[i].value()) {
            firstTitleIndex = i;
        }
        if (firstTitleIndex > 0 && lastTitle == texts[i].value()) {
            lastTitleIndex = i;
            break;
        }
    }
    ...
    // 数据，按钮 "选择","操作"不属于数据，所以开始下标+2
    var data = [], j = 0, data1;
    for (var i = lastTitleIndex + 2; i < texts.length; i++) {
        if (j = 0) {
            data1 = new Array(titleTotal);
        }
        data1[titles[j]] = texts[i].value();
        j++;
        if (j == titleTotal) {
            data.push(data1);
            j = 0;
