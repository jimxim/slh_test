...
/**
 * 尺码表头明细 {"明细" : [ { "货品" : "k300", "颜色" : "米色","尺码" :{"T9":9,"S3":3} ]}
 * @param o
 */
function editSalesBillDetSizhead(o) {

    // debugObject(o1);
    var details = o["明细"], mergeLine = 0;
    for ( var i in details) {
        var o1 = getDetSizheadTitle(), colIndex=0;
        var tfNum = o1["明细输入框个数"];
        var start = tfNum * (i - mergeLine);
        var d = details[i];
        var f0 = new TField("货品", TF_AC, start + 0, d["货品"], -1, 0);
        var view1 = getScrollView();
        ...
        colIndex = o1["颜色"];
        var f1 = new TField("", TF_AC, start + 0, d["货品"], -1, 0);
...
function getDetSizheadTitle() {
    ...
    for (i = qrTitle2.index + 1; i < texts.length; i++) {
        ...
        if (!isNull(t) && isDefined(t) && !isNull(size) && isDefined(size)) {
            ...
            logInfo("some illegal size=" + size + " t=" + t);
        }
