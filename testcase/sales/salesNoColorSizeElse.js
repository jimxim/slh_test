...
function testSalesNoColorSizeElse001() {
    ...
    run("【销售开单－销售汇总-按金额汇总】翻页_排序_汇总", "test170307_170306");
...
function testSalesNoColorSizeElseAll_2() {
    ...
    run("【销售开单-按汇总】按配货员汇总", "test170633_1");
    run("【销售开单-按汇总】按配货员汇总", "test170633_2");
...
function test170264() {
    ...
    var ret = isAnd(isEqual("部分发货", a), isEqual("15", qr.data[0]["订货数"]),
            ...
            isEqual(0, a1), isEqual("-1", a2), isEqual("0", a3), isEqual("0",
                    a4), isEqual("1", getTextFieldValue(getScrollView(), 30)),
            isEqual("1", getTextFieldValue(getScrollView(), 31)), isEqual("0",
...
function test170306_1() {
    ...
    tapMenu("销售开单", "按汇总", "按金额汇总");
    var keys = { "day1" : getDay(-30), "day2" : getToday(), "门店" : "常青店" };
    ...
    tapMenu("销售开单", "按汇总", "按金额汇总");
    var keys1 = { "day1" : getDay(-30), "day2" : getToday(), "门店" : "常青店" };
    ...
    logDebug("ds=" + ds + ", sk=" + sk + ", ret=" + ret + ", ret1=" + ret1
...
function test170308() {
    ...
    keys = { "日期从" : getDay(-30), "厂商" : "vell", "款号" : "3035",
...
function test170339_170705() {
    tapMenu("销售开单", "按汇总", "按客户上货");
    var keys = { "客户" : "ls", "上架从" : "2015-01-01", "上架到" : getToday(),
        "款号" : "k300", "品牌" : "Adidas", "日期从" : getDay(-10), "类别" : "登山服",
        "季节" : "春季" };
    var fields = salesCustomerSupplyFields(keys);
    query(fields);
...
function test170371() {
    ...
    var ret = isAnd(isEqual(1400, b), isEqual("东灵测试-银行账户", b1), isEqual("代收收款",
            qr1.data[0]["类型"]), isAqualOptime(getOpTime(), qr1.data[0]["操作日期"],
            2), isEqual(64, qr1.data[1]["金额"]), isEqual("东灵测试-现金账户",
            qr1.data[1]["账户"]), isEqual("代收收款", qr1.data[1]["类型"]),
            isAqualOptime(getOpTime(), qr1.data[1]["操作日期"], 2));
...
function test170479() {
    ...
    var ret2 = isAnd(isEqual(getToday(), getTextFieldValue(window, 9)), isIn(
            getTextFieldValue(getScrollView(), 0), r));
...
function test170570_170577_170579() {
    ...
    tap(app.navigationBar().buttons()["全 选"]);
    tap(app.navigationBar().buttons()["完成"]);
    ...
    tap(app.navigationBar().buttons()["全 选"]);
    delay();
    tap(app.navigationBar().buttons()["不 选"]);
    delay();
    tap(app.navigationBar().buttons()["完成"]);
    delay();

    var ret3 = isEqual(0, getTextFieldValue(window, 5));
...
function test170603() {
    ...
    var ret = isAnd(isEqual(0, getTextFieldValue(getScrollView(), 5)),
            !isEqual("-7", getTextFieldValue(getScrollView(), 5)));
...
function test170633_1() {
    ...
    for (var i = 0; i < 4; i++) {
        if (i == 0 || i == 1) {
            ...
            ret1 = ret1 && isEqual("", getTextFieldValue(window, i));
        }
    }
...
function test170634() {
    ...
    var ret = isAnd(isEqual(100, sub(xj1, xj)), isEqual(0, sub(sk1, sk)),
            isEqual(0, sub(hk1, hk)), isEqual(0, sub(sxs1, sxs)), isEqual(100,
                    sub(sxe1, sxe)));
...
function test170635() {
    ...
    var ret1 = isEqual("预付款", qr.data[0]["备注"]);
...
function test170643() {
    ...
    var ret = isAnd(isEqual("", a), isEqual(batch, qr.data[0]["批次"]));
...
function test170645() {
    tapMenu("销售开单", "按挂单");
    var keys = { "日期从" : "2015-01-10", "日期从" : getDay(-1) };
    var fields = salesQueryGuaDanFields(keys);
    query(fields);
...
function test170669() {
    ...
    var ret = isAnd(isEqual(0, getTextFieldValue(getScrollView(), 5)),
            !isEqual("-10", getTextFieldValue(getScrollView(), 5)));
...
function test170701() {
    ...
    var ds1 = getTextFieldValue(window, 8);
    var xj = getTextFieldValue(getScrollView(), 12);
