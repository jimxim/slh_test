...
function testSalesNoColorSizeAll_2() {
    ...
    // run("【销售开单-开单】是否要弹出价格刷新窗口--客户从A切换到B", "test170619_170620");
...
function testSalesNoColorSize001() {
    ...
    run("【销售开单－开单】使用上次成交价", "test170107_170529");
...
function testSalesNoColorSize001_1() {
    ...
    run("【销售开单－开单】异地发货－－后台不绑仓库，开单时不选择发货仓库", "test170120_170762");
...
function testSalesNoColorSize001_3() {
    ...
    run("【销售开单-开单】积分跨门店共享", "test170183_170185");
    ...
    run("【销售开单-新增】核销/退货", "test170746_1");
...
function test170054_1() {
    ...
    query();
    var qr = getQR();
    var ret1 = isAnd(isEqual(Number(totalMoney) + Number(dikou),
            qr.data[0]["金额"]), isEqual(totalMoney, qr.data[0]["现金"]), isEqual(
            -dikou, qr.data[0]["还款/抵扣"]));
...
function test170065_1() {
    // 核销欠款，不输入款号
    tapMenu("销售开单", "开  单+");
    var r = "anewkh" + randomWord(false, 8);
    var json = { "名称" : r };
    editQuickAddCustomer(json);
...
function test170077_1_170067() {
    ...
    tapMenu("销售开单", "开  单+");
    var json = { "明细" : [ { "货品" : "3035", "数量" : 1, "折扣" : 0.8 } ],
        "onlytest" : "yes" };
    editSalesBillNoColorSize(json);
    saveAndAlertOk();
    tapPrompt();
    var alertMsg1 = getArray1(alertMsgs, -1);
    var ret5 = (isIn(alertMsg1, "输入的折扣低于拿货折扣"));
    tapReturn();
...
function test170078() {
    ...
    var json = { "客户" : "lx" };
    editSalesBillCustomer(json);
    var ret3 = isAnd(isIn(alertMsg, "是否需要重新刷新明细价格等信息"));
    saveAndAlertOk();
    tapPrompt();
    var alertMsg1 = getArray1(alertMsgs, -1);
    ret3 = isAnd(ret3, isIn(alertMsg1, "金额汇总数据错误"));

    tapButton(getScrollView(-1), 0);
    var json = { "明细" : [ { "货品" : "3035", "数量" : 1 } ], "特殊货品" : { "免单" : 2 },
        "折扣" : 0.58 };
    editSalesBillNoColorSize(json);
...
function test170080_170084() {
    ...
    qr = getQRDet();
    tapReturn();
    var ret5 = isEqual(r1, qr.data[0]["折扣"]);
...
function test170092() {
    ...
    var f = new TField("物流商", TF_AC, -4, "undefinedKD", -1, 0);
    var fields = [ f ];
    setTFieldsValue(window, fields);
    tapNaviRightButton();
    tapPrompt();
    ...
    var ret4 = isAnd(isIn(alertMsg1, "必须选择物流商"));
    tapNaviLeftButton();

    json = { "物流商" : "tt", "运单号" : 15068165766, "备注" : "界面检查" };
    ...
    logDebug(" ret=" + ret + ", ret2=" + ret2 + ", ret3=" + ret3 + ", ret4="
            + ret4 + ",value1=" + value1 + ", value2=" + value2 + ", value3="
...
function test170097() {
    ...
    tapMenu("销售开单", "按明细查");
    var keys = { "款号" : "8989", "客户" : "ls", "门店" : "常青店", "类型" : "换码",
        "备注" : "换码" };
    var fields = salesQueryParticularFields(keys);
    query(fields);
    var qr = getQR();
    var ret1 = isAnd(isEqual("李四", qr.data[0]["客户"]), isEqual("8989",
...
function test170183_170185() {
    ...
    var json = { "明细" : [ { "货品" : "3035", "数量" : "1" } ], "不返回" : "yes" };
    editSalesBillNoColorSize(json);
    var qr1 = json["输入框值"];
    var money = qr1["总计"];
    ...
    editSalesBillCustomer(json);
    tapButton(window, "核销");
    var b1 = getStaticTextValue(getScrollView(-1, 0), 1);
    var c1 = b1.split(": ");
    tapNaviLeftButton();
    tapReturn();
    var ret1 = isAnd(isEqual(a, c[1]),
            isEqual(Number(a) + Number(money), c1[1]));

    tapMenu("往来管理", "客户查询");
    tapButton(window, QUERY);
    var qr2 = getQR();
    var a1 = qr2.data[0]["当前积分"];

    tapMenu("往来管理", "积分查询");
    tapButton(window, QUERY);
    qr = getQR();
    var totalScore = qr.counts["当前积分"];
    var len1 = qr.data.length;
    var ret2 = isAnd(isEqual(money, sub(a1, a)), !isEqual(1, len), !isEqual(1,
            len1), isEqual(a1, totalScore));
...
function test170184() {
    ...
    tapNaviLeftButton();
    tapReturn();
    var ret1 = isAnd(isEqual(s, c[1]),
            isEqual(Number(s) + Number(money), c1[1]));
    ...
    var ret2 = isAnd(isEqual(money, sub(s1, s)), !isEqual(a, s), !isEqual(a1,
            s1));
...
function test170188() {
    ...
    query(fields);
    // 综合收支表
    var x1 = getSACountsQR("现", "支出", "积分兑换");
    tapNaviLeftButton();
    ...
    tapButton(window, QUERY);
    // 综合收支表
    var x2 = getSACountsQR("现", "支出", "积分兑换");
    tapNaviLeftButton();
    var ret = isEqual(r, Number(x2) - Number(x1));
    ...
    var qr = getQR();
    var s = qr.data[0]["金额"];
    var exp = { "类型" : "积分兑换", "账户" : "东灵测试-现金账户", "金额" : -r, "操作人" : "总经理" };
    var ret1 = isAnd(isEqualObject(exp, qr.data[0]));

    // 判断综合收支表金额大于0，收支流水金额小于0
    var ret2 = false;
    if (x2 >= 0 && s <= 0) {
        ret2 = true;
    }
...
function test170239() {
    var qo = { "备注" : "开单是否门店过滤人员" };
    var o = { "新值" : "1", "数值" : [ "支持，开启后店员只显示本门店人员", "in" ] };
    setGlobalParam(qo, o);
    runAndAlert("test210020Clear", OK);
    delay(5);
    var menu = { "采购入库" : "新增入库+", "采购订货" : "新增订货+", "销售订货" : "新增订货+",
        "销售开单" : ADDBILL };
    return test170239Field(menu);
}
function test170239Field(menu) {
...
function test170247Field(menu, params, colorSize) {
    ...
    for (var j = 0; j < params.length; j++) {
        ...
        tapMenu("销售开单", "开  单+");
        var r1 = randomWord(false, 7);
        var r = "anewkh" + r1;
        var o = { "名称" : r, "手机" : r1, "店员" : "000,", "地址" : r };
        editQuickAddCustomer(o, "yes");
        ...
        query(fields);
        var qr1 = getQR();
        var ret1 = isAnd(isEqual(r, qr1.data[0]["客户"]), isEqual(price,
                qr1.data[0]["单价"]));
...
function test170248() {
    var params = [ { "新值" : "2", "数值" : [ "默认打包价", "in" ] },
            { "新值" : "1", "数值" : [ "默认零批价", "in" ] } ];
    var menu = { "销售订货" : "新增订货+", "销售开单" : ADDBILL };
    return test170248Field(menu, params);
}
function test170248Field(menu, params) {
    var ret = true, qr, price, keys;
    var keys1 = { "价格类型" : "零批价" };
    var keys2 = { "价格类型" : "打包价" };
    var qo = { "备注" : "默认显示价格类型" };
    for (var j = 0; j < params.length; j++) {
        ...
        var r1 = randomWord(false, 7);
        var r = "anewkh" + r1;
        var o1 = { "名称" : r, "手机" : r1, "店员" : "000,", "适用价格" : "零批价",
            "地址" : r1 };
        var o2 = { "名称" : r, "手机" : r1, "店员" : "000,", "适用价格" : "打包价",
            "地址" : r1 };
        if (params[j]["新值"] == 1) {
            o = o2;
            price = 180;
...
function test170542() {
    ...
    tapFirstText();
    tapStaticText(window, "代收");
    var ret1 = isAnd(isEqual(wls, getTextFieldValue(window, -4)), isEqual(ydh,
    ...
    query(fields);
    var qr = getQR();
    var ret2 = isAnd(isEqual(getToday("yy"), qr.data[0]["日期"]), isEqual("天天物流",
...
function test170559_170697() {
    ...
    var alertMsg1 = getArray1(alertMsgs, -1);
    var alertMsg2 = getArray1(alertMsgs, -2);
    var ret4 = isIn(alertMsg1, "只能修改支付方式") || isIn(alertMsg2, "只能修改支付方式");
...
function test170619_170620() {
    var menu = { "采购订货" : "新增订货+", "销售开单" : ADDBILL };
    return test170619Field(menu);
}
function test170619Field(menu) {
    var qo, o, ret = true;
    qo = { "备注" : "刷新窗口" };
    o = { "新值" : "1", "数值" : [ "默认刷新", "in" ] };
    ret = isAnd(ret, setGlobalParam(qo, o));
    ...
    var json = { "客户" : "ls", "明细" : [ { "货品" : "k300", "数量" : "10" } ],
        "onlytest" : "yes" };
    var json1 = { "明细" : [ { "货品" : "k300", "数量" : "10" } ], "onlytest" : "yes" };
    for (var j = 0; j < params.length; j++) {
        setGlobalParam(qo, params[j]);
        for ( var menu1 in menu) {
            var cus = "ls";
            ...
            var o = { "客户" : "lt" };
            editSalesBillCustomer(o);
            ret1 = isAnd(ret1, isIn(alertMsg, "是否需要重新刷新明细价格等信息"));
            tapPrompt();
            tapReturn();
    ...
    loDebug(" ret=" + ret + " ret1=" + ret1 + " ret2=" + ret2);
    return ret && ret1 && ret2;
}
function test170621() {
    var qo, o, ret = true;
    ...
    tapButton(window, CLEAR);
    var qr = getQRDet();
    var ret1 = isAnd(!isIn(alertMsg, "是否需要重新刷新明细价格等信息"), isEqual(price,
            qr.data[0]["单价"]));

    var o = { "客户" : "lt" };
    editSalesBillCustomer(o);
    qr = getQRDet();
    var ret2 = isAnd(!isIn(alertMsg, "是否需要重新刷新明细价格等信息"), isEqual(price,
            qr.data[0]["单价"]));
    tapPrompt();
    tapReturn();
    ...
    tapButton(window, CLEAR);
    qr = getQRDet();
    var ret3 = isAnd(!isIn(alertMsg, "是否需要重新刷新明细价格等信息"), isEqual(price,
            qr.data[0]["单价"]));

    o = { "客户" : "lt" };
    editSalesBillCustomer(o);
    qr = getQRDet();
    var ret4 = isAnd(!isIn(alertMsg, "是否需要重新刷新明细价格等信息"), isEqual(price,
            qr.data[0]["单价"]));
    tapPrompt();
    tapReturn();
...
function test170690() {
    ...
    query();
    tapFirstText();
    var m2 = (Number(a1) / 100).toFixed(2);
    o = { "特殊货品" : { "积分抵现" : m2 } };
    editSalesBillSpecial(o);
    saveAndAlertOk();
    tapPrompt();
...
function test170691() {
    ...
    query();
    tapFirstText();
    var m2 = ((Number(a) + 100) / 100).toFixed(2);
    o = { "特殊货品" : { "积分抵现" : m2 } };
    editSalesBillSpecial(o);
    saveAndAlertOk();
    tapPrompt();
...
function test170693() {
    ...
    var qr = getQRverify(texts, "序号", 0, titleTexts);
    // 检查底部数据的可见性
    var l = qr.data.length;
    var idx = l * 7 - 7;
    var text = getStaticTextValue(getScrollView(-1), idx);
    var index = getArrayIndexIn(texts, text);
    var obj = getStaticText(getScrollView(-1), index);
    obj.scrollToVisible();
    var ret2 = false;
    if (!isUIAElementNil(obj) && obj.isVisible()) {
        ret2 = true;
    }
    tapNaviLeftButton();
    tapNaviLeftButton();
...
function test170714() {
    ...
    var menu = { "采购入库" : "按订货入库", "销售开单" : "按订货开单" };
    var ret6 = false;
    for (menu1 in menu) {
        tapMenu(menu1, menu[menu1]);
        query();
...
function test170732() {
    ...
    tapMenu("销售开单", "开  单+");
    var json = { "客户" : "ls", "明细" : [ { "货品" : "3035", "数量" : 10, "折扣" : r } ] };
    editSalesBillNoColorSize(json);
    ...
    tapMenu("销售订货", "新增订货+");
    var json = { "客户" : "ls", "明细" : [ { "货品" : "k300", "数量" : 10, "折扣" : r } ] };
    editSalesBillNoColorSize(json);
    ...
    tapMenu("销售开单", "开  单+");
    var json = { "客户" : "ls", "明细" : [ { "货品" : "3035", "数量" : 1, "折扣" : r } ] };
    editSalesBillNoColorSize(json);
    ...
    tapMenu("销售订货", "新增订货+");
    var json = { "客户" : "ls", "明细" : [ { "货品" : "k300", "数量" : 1, "折扣" : r } ] };
    editSalesBillNoColorSize(json);
...
function test170743_170744() {
    var params = [ { "新值" : "0", "数值" : [ "显示颜色尺码表", "in" ] },
            { "新值" : "1", "数值" : [ "默认均色均码", "in" ] } ];
    var menu = { "采购入库" : "新增入库+", "采购订货" : "新增订货+", "销售订货" : "新增订货+",
        "销售开单" : ADDBILL };
    return test170743_170744Field(menu, params);
}
function test170743_170744Field(menu, params) {
    var qo, o, ret = true;
    qo = { "备注" : "优先结余还是优先找零" };
    o = { "新值" : "1", "数值" : [ "优先找零" ] };
    ret = isAnd(ret, setGlobalParam(qo, o));
    ...
    var json = { "客户" : "lx", "明细" : [ { "货品" : "3035", "数量" : 1 } ],
        "现金" : 1000 };
    editSalesBillNoColorSize(json);
    var ret5 = isAnd(isIn(alertMsg, "保存成功"));

    tapMenu("销售订货", "新增订货+");
    editSalesBillNoColorSize(json);
    var ret6 = isAnd(isIn(alertMsg, "保存成功"));
    ...
    logDebug(", ret=" + ret + ", ret1=" + ret1 + ", ret5=" + ret5 + ", ret6="
            + ret6 + ", ret9=" + ret9 + ", ret10=" + ret10 + ", ret11=" + ret11
            + ", ret12=" + ret12);
    return ret && ret1 && ret5 && ret6 && ret9 && ret10 && ret11 && ret12;
}
function test170745() {
    var qo, o, ret = true;
    qo = { "备注" : "开单模式" };
...
function test170746_1() {
    var menu = { "销售订货" : "新增订货+", "销售开单" : ADDBILL };
    return test170746Field(menu, "no");
}
function test170746_2() {
    var menu = { "销售订货" : "新增订货+", "销售开单" : ADDBILL };
    return test170746Field(menu, "yes");
}
function test170746Field(menu, colorSize) {
    var qo, o, ret = true;
    qo = { "备注" : "优先结余还是优先找零" };
    o = { "新值" : "1", "数值" : [ "优先找零" ] };
    ret = isAnd(ret, setGlobalParam(qo, o));
    ...
    if (colorSize == "no") {
        ...
        debugArray(alertMsgs);
        alertMsg1 = getArray1(alertMsgs, -2);
        ret3 = isAnd(ret3, isIn(alertMsg1, "现金栏必须有支付并大于找零"));
    } else {
        tapReturn();
    }
...
function test170747_1() {
    return test170747Field(true);
}
function test170747_2() {
    return test170747Field(false);
}
function test170747Field(rights) {
    var qo, o, ret = true;
    qo = { "备注" : "开单时挂单提醒" };
    o = { "新值" : "1", "数值" : [ "开启" ] };
    ret = isAnd(ret, setGlobalParam(qo, o));
    ...
    debugArray(alertMsgs);
    alertMsg1 = getArray1(alertMsgs, -1);
    var ret6 = isIn(alertMsg1, "当前客户存在挂单");
    qr = getQRtable1(getScrollView());
    len = qr.data.length;
    var ret4 = true, ret5 = false;
    for (var i = 0; i < len - 1; i++) {
        var customer = qr.data[i]["客户"];
        if (!isEqual("李四", customer)) {
            ret4 = false;
...
function test170751() {
    ...
    setTFieldsValue(window, fields);
    tapButton(window, RETURN);
    var ret3 = !isIn(alertMsg, "确定返回吗");
    var btn = getButton(window, RETURN);
    if (isUIAButton(btn)) {
        ret3 = isAnd(ret3, ret3 = false);
        tapButtonAndAlert(RETURN, OK);
    }

    tapMenu("销售开单", "开  单+");
    var json = { "客户" : "ls", "备注" : "zdbz", "onlytest" : "yes" };
    editSalesBillNoColorSize(json);
    var o = [ { "备注" : [ "<>" ] } ];
    editChangeSalesBillOrderRemarks(o, "no");
    tapButtonAndAlert(RETURN, OK);
    ...
    debugArray(alertMsgs);
    var alertMsg1 = getArray1(alertMsgs, -1);
    var ret1 = (isIn(alertMsg1, "确定返回吗"));

    tapMenu("销售开单", "开  单+");
    var keys = { "现金" : 100, "刷卡" : "1.56" };
    var fields = editSalesBillFields(keys);
    setTFieldsValue(window, fields);
    tapButtonAndAlert(RETURN, OK);
    debugArray(alertMsgs);
    var alertMsg1 = getArray1(alertMsgs, -1);
    var ret2 = isIn(alertMsg1, "确定返回吗");

    logDebug(" ret=" + ret + ", ret1=" + ret1 + ", ret2=" + ret2 + ", ret3="
            + ret3);
    return ret && ret1 && ret2 && ret3;
}
function test170752_1() {
    var menu = { "采购入库" : "新增入库+", "销售订货" : "新增订货+", "销售开单" : ADDBILL };
    return test170752Field(menu, false);
}
function test170752_2() {
    var menu = { "采购入库" : "新增入库+", "销售订货" : "新增订货+", "销售开单" : ADDBILL };
    return test170752Field(menu, true);
}
function test170752Field(menu, rights) {
    var qo, o;
    qo = { "备注" : "挂单是否打印" };
    o = { "新值" : "1", "数值" : [ "打印" ] };
    setGlobalParam(qo, o);
    ...
    for ( var menu1 in menu) {
        ...
        var json = { "客户" : cus, "明细" : [ { "货品" : "3035", "数量" : 1 } ],
            "onlytest" : "yes" };
        editSalesBillNoColorSize(json);
        var o1 = { "保存成功，是否打印" : OK };
        setValueToCache(ALERT_MSG_KEYS, o1);
        ...
        var alertMsg1 = getArray1(alertMsgs, -1);
        var alertMsg2 = getArray1(alertMsgs, -2);
        ret = isAnd(ret, isIn(alertMsg1, "是否打印") || isIn(alertMsg2, "是否打印"));
        ...
        alertMsg1 = getArray1(alertMsgs, -1);
        alertMsg2 = getArray1(alertMsgs, -2);
        ret1 = isAnd(ret1, isIn(alertMsg1, "是否打印") || isIn(alertMsg2, "是否打印"));
        ...
        if (rights) {
            ...
            saveAndAlertOk();
            tapPrompt();
            ret2 = isAnd(ret2, !isIn(alertMsg, "已经打印的单据"));
            tapReturn();
        }
    }
...
function test170758() {
    tapMenu("销售订货", "新增订货+");
    var o = { "客户" : "ls",
        "明细" : [ { "货品" : "3035", "数量" : 20 }, { "货品" : "k300", "数量" : 20 } ] };
    var o1 = {
        "客户" : "ls",
        "明细" : [ { "货品" : "3035", "数量" : 20 }, { "货品" : "k300", "数量" : [ 30 ] } ] };
    if ("colorSize" == "yes") {
        o = o1;
    }
    ...
    query();
    var qr = getQR();
    var ret = isEqual(20, qr.data[0]["已发"]);
