...
function testSalesNoColorSizeElseAll_2() {
    ...
    run("【销售开单-按挂单】前几天的挂单保存之后日期应变为当天", "test170725");
...
function test170005() {
    ...
    var ys = getTextFieldValue(window, cashTFindex + 2);
    saveAndAlertOk();
    tapPrompt();
    tapReturn();
...
function test170028_170038() {
    tapMenu("销售开单", "开  单+");
    var json = { "客户" : "lt", "特殊货品" : { "抹零" : 100 }, "onlytest" : "yes" };
    editSalesBillNoColorSize(json);
    ...
    tapMenu("销售开单", "按明细查");
    query();
    var keys = { "款号" : "00000", "款号名称" : "抹零", "客户" : "lt", "店员" : "000,",
    ...
    var ret = isAnd(isEqual("李天", b), isEqual("00000", b1), isEqual(0, b2),
            isEqual(100, b3));
...
function test170577() {
    ...
    debugArray(alertMsgs);
    var alertMsg1 = getArray1(alertMsgs, -2);
    var ret3 = isIn(alertMsg1, "客户或厂商错误");
...
function test170718() {
    ...
    var ret = isAnd(isEqual(2, sub(ths1, ths)), isEqual(1, sub(nhs1, nhs)));
    ...
    changeTFieldValue(qFields["备注"], "预付款");
    setTFieldsValue(window, qFields);
    tapButton(window, QUERY);
    qr3 = getQR();
    var cs1 = qr3.total;
    var ret2 = isEqual(nhs1, sub(cs, cs1));
...
function test170725() {
    ...
    ret1 = isAnd(ret1, isEqual("k300", qr.data[0]["款号"]), isEqual(-2,
            qr.data[0]["数量"]));
