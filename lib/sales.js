...
function editSalesBillField1(o, key) {
    ...
    if (isDefined(v)) {
        ...
        var fields = editSalesBillFields(keys);
        setTFieldsValue(window, fields);
        // 备注框输入内容超过5个时会触发，进入后重新输入一次值，针对备注框已经有值，而输入的内容不同的情况
        // if (key == "备注") {
        // var view = getPop();
...
function editSalesBillSpecial(o) {
    ...
    if (isDefined(ot)) {
        ...
        var n4 = ot["不核算积分"];
        if (isDefined(n4)) {
            fields.push(new TField("不核算积分", TF, 4, n3));
        }
        var n7 = ot["积分抵现"];
        if (isDefined(n7)) {
...
function editSalesBillGetValue(o) {
    ...
    index = getEditSalesTFindex("客户,厂商", "结余");// 7.23新增 优先结余
    if (index >= 0) {
        arr["结余"] = getTextFieldValue(window, index);
    }
    ...
    index = getEditSalesTFindex("客户,厂商", "日期");
    if (index >= 0) {
        arr["日期"] = getTextFieldValue(window, index);
    } else {
        arr["日期"] = getTextFieldValue(window, cardTFindex + 2);
    }
    ...
    index = getEditSalesTFindex("客户,厂商", "订货门店");
    if (index >= 0) {
    ...
    arr["总计"] = getTextFieldValue(window, remitTFindex - 1);
...
function editSalesBillSave(o) {
    ...
    o["输入框值"] = editSalesBillGetValue(o);
...
function editSalesBillAddCustomer(o) {
    ...
    n = o["店员"];
    if (isDefined(n)) {
        fields.push(new TField("店员", TF_AC, 2, n, -1, 0));// 不稳定，有时店员输入不了，可能是键盘未触发
    }
    n = o["适用价格"];
    if (isDefined(n)) {
...
/**
 * 开单模式20,增加配货员输入框
 * @param o eg: {"配货员":"000,总经理"}
 */
function editSalesBillMacth(o) {
    if (isDefined(o["配货"])) {
        var f8 = new TField("配货", TF_AC, 8, o["配货"], -1, 0);
        var fields = [ f8 ];
        setTFieldsValue(window, fields);
    }
    return o;
...
function getDetSizheadTitle() {
    ...
    var title2 = "操作";
    logDebug("明细标题  title1=" + title1 + "   title2=" + title2);
