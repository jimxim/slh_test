...
function testSalesNoColorSizeElse001() {
    run("【销售开单－按批次查】翻页_排序_汇总", "test170001_1_1700010_1700011_1700012");
    ...
    // run("【销售开单－销售汇总-按客户上货】按客户上货", "test170338");// (商路花程序改动，在确认是否需要修改)
...
function test170001_1_1700010_1700011_1700012() {
    ...
    tapMenu("销售开单", "按批次查");
    var keys = { "日期从" : getDay(-3), "日期到" : getToday(), "作废挂单" : "正常" };
...
function test170291() {
    ...
    target.frontMostApp().mainWindow().tableViews()[5].groups()["批次"]
            .staticTexts()["门店"].tapWithOptions({ tapOffset : { x : 0.44,
        y : 0.52 } });
    var qr = getQRtable1(window, 8, -2);
    var ret = true;
    var len = qr.data.length;
    for (var i = 0; i < len - 1; i++) {
        var md = qr.data[i]["门店"];
        if (isEqual(md, "仓库店") || isEqual(md, "中洲店")) {
            ret = false;
        }
    }
...
function test170305_2() {
    ...
    var ret1 = isAnd(isEqual(getToday("yy"), a1), isEqual("常青店", a2), isEqual(
            "李四", a3), isEqual("总经理", a4), isEqual("710", a5),
            isEqual("3", a6), isEqual("402", a7), isEqual("", a8), isEqual(r,
                    a9), isAqualOptime(a10, getOpTime()), isEqual("62", a11),
            isEqual("600", a12), isEqual("400", a13), isEqual("0", a14),
            isEqual("50", a14));
...
function test170313() {
    ...
    tapMenu("销售开单", "按明细查");
    var keys = { "门店" : "常青店", "日期从" : getDay(-30), "到" : getToday(),
        "类型" : "退货" };
    var fields = salesQueryParticularFields(keys);
    query(fields);
    qr = getQR();
    ...
    var ret1 = isAnd(isEqual(add(totalReturn, totalReturn1), qr.counts["退数"]),
            isEqual(add(totalMoney, totalMoney1), qr.counts["退额"]));
...
function test170319_1() {
    ...
    tapFirstText();
    var qr1 = getQR2(getScrollView(-1, 0), "款号", "数量");
    var kh = qr1.data[0]["款号"];
    var num = qr1.data[0]["数量"];
    var kh1 = qr1.data[1]["款号"];
    var num1 = qr1.data[1]["数量"];
    // var kh2 = qr1.data[2]["款号"];
    // var num2 = qr1.data[2]["数量"];
    var kh3 = qr1.data[3]["款号"];
    var num3 = qr1.data[3]["数量"];
    // var kh4 = qr1.data[4]["款号"];
    // var num4 = qr1.data[4]["数量"];
    ...
    tapMenu("销售开单", "按明细查");
    var keys = { "日期从" : getDay(-30), "到" : getToday(), "款号" : "00000",
        "门店" : "常青店", "店员" : "000" };
    var fields = salesQueryParticularFields(keys);
    query(fields);
    qr = getQR();
    ...
    tapMenu("销售开单", "按明细查");
    var keys = { "日期从" : getDay(-30), "到" : getToday(), "款号" : "00001",
        "门店" : "常青店", "店员" : "000" };
    var fields = salesQueryParticularFields(keys);
    query(fields);
    qr = getQR();
    ...
    tapMenu("销售开单", "按明细查");
    var keys = { "日期从" : getDay(-30), "到" : getToday(), "款号" : "k300",
        "门店" : "常青店", "店员" : "000" };
    var fields = salesQueryParticularFields(keys);
    query(fields);
    qr = getQR();

    var totalSl1 = qr.counts["数量"];

    // tapMenu("销售开单", "按明细查");
    // var keys = { "日期从" : getDay(-30), "到" : getToday(), "款号" : "8989",
    // "门店" : "常青店", "店员" : "000" };
    // var fields = salesQueryParticularFields(keys);
    // query(fields);
    // qr = getQR();
    //
    // var totalSl2 = qr.counts["数量"];, isEqual("8989", kh2), isEqual(totalSl2,
    // num2) isEqual("00001", kh4), isEqual(totalSl4, num4)

    tapMenu("销售开单", "按明细查");
    var keys = { "日期从" : getDay(-30), "到" : getToday(), "款号" : "4562",
        "门店" : "常青店", "店员" : "000" };
    var fields = salesQueryParticularFields(keys);
    query(fields);
    qr = getQR();

    var totalSl3 = qr.counts["数量"];

    var ret2 = isAnd(isAqualNum(-ml1, totalXj, 0.5), isAqualNum(qt1, totalXj1,
            0.5), isEqual("3035", kh), isEqual(totalSl, num), isEqual("k300",
            kh1), isEqual(totalSl1, num1), isEqual("4562", kh3), isEqual(
            totalSl3, num3));
...
function test170325_1() {
    ...
    tapMenu("销售开单", "按批次查");
    var keys = { "日期从" : getDay(-30), "日期到" : getToday(), "客户" : "ls" };
    var fields = salesQueryBatchFields(keys);
    query(fields);
    var qr = getQR();
...
function test170332_170337() {
    ...
    var keys = { "客户" : r, "日期从" : getDay(-30), "到" : getToday() };
    ...
    tapFirstText();
    qr = getQR2(getScrollView(-1, 0), "批次", "操作日期");
    var a = qr.data[0];
    var b = qr.data[1];

    var result = getQRStaticTexts(getScrollView(-1, 0), "批次", "操作人");
    var batch = result.data["批次"];
    var time = result.data["日期"];
    var num = result.data["总数"];
    var totalMoney = result.data["总额"];
    var type = result.data["类型"];
    var tip = result.data["备注"];
    var staff = result.data["操作人"];

    var result1 = getQR3(getScrollView(-1, 0), "序号", "小计");
    var kh = result1[0]["款号"];
    var kh1 = result1[1]["款号"];
    var kh2 = result1[2]["款号"];
    var kh3 = result1[3]["款号"];

    var sl = result1[0]["数量"];
    var sl1 = result1[1]["数量"];
    var sl2 = result1[2]["数量"];
    var sl3 = result1[3]["数量"];

    var money = result1[0]["小计"];
    var money1 = result1[1]["小计"];
    var money2 = result1[2]["小计"];
    var money3 = result1[3]["小计"];

    tapNaviLeftButton();
    tapNaviLeftButton();

    tapMenu("销售开单", "按批次查");
    var keys = { "客户" : r, "日期从" : getDay(-30), "日期到" : getToday(),
        "是否未结" : "是", "作废挂单" : "正常" };
    var fields = salesQueryBatchFields(keys);
    query(fields);
    qr = getQR();
    ...
    var a1 = qr.data[0];
    var b1 = qr.data[1];
    ret = isAnd(ret, isEqualObject(a, a1), isEqualObject(b, b1), isEqual(batch,
            batch1), isEqual(time, getToday()), isEqual(num, qr.data[0]["数量"]),
            isEqual(totalMoney, qr.data[0]["金额"]), isEqual("销售出货", type),
            isEqual("(5;-5)", tip), isEqual(staff, qr.data[0]["店员"]));

    tapFirstText();

    ret = isAnd(ret, isEqual(kh, getTextFieldValue(getScrollView(), 0)),
            isEqual(sl, getTextFieldValue(getScrollView(), 3)), isEqual(money,
                    getTextFieldValue(getScrollView(), 5)), isEqual(kh1,
                    getTextFieldValue(getScrollView(), 7)), isEqual(sl1,
                    getTextFieldValue(getScrollView(), 10)), isEqual(money1,
                    getTextFieldValue(getScrollView(), 12)), isEqual(kh2,
                    getTextFieldValue(getScrollView(), 14)), isEqual(sl2,
                    getTextFieldValue(getScrollView(), 17)), isEqual(money2,
                    getTextFieldValue(getScrollView(), 19)), isEqual(kh3,
                    getTextFieldValue(getScrollView(), 14)), isEqual(sl3,
                    getTextFieldValue(getScrollView(), 17)), isEqual(money3,
                    getTextFieldValue(getScrollView(), 19)));
    ...
    var ret1 = isAnd(isEqual(wj1, sum1), isEqual(xj11, qr.counts["现金"]),
            isEqual(sk11, qr.counts["刷卡"]), isEqual(hk1, qr.counts["汇款"]),
            isEqual(ds1, qr.counts["代收"]), isEqual(sxs1, qr.counts["数量"]),
            isEqual(sxe1, qr.counts["金额"]));
    ...
    var ret1 = isAnd(ret1, isEqual(xs1, add(ts1, sxs1)));
...
function test170356() {
    ...
    var ret = isAnd(isEqual(10, sub(xs1, xs)), isEqual(1, sub(ts1, ts)),
            isEqual(9, sub(sxs1, sxs)), isEqual(1750, sub(sxe1, sxe)));
...
function test170366() {
    ...
    tapFirstText(getScrollView(), "序号", 20);
    var k4 = getTextFieldValue(window, 4);
    var ret = isAnd(isEqual(add(zk * price * num, 6), k4));
    ...
    var ret1 = isAnd(isEqual(add(zk * price * num, 6), a), isEqual("汇通快递", a1),
            isEqual("123", a2), isEqual("a", a3), isEqual("是", a4), isEqual(r,
                    a5), isEqual("常青店", a6), isEqual("否", a7));
...
function test170368() {
    ...
    var ret1 = isAnd(isEqual("500", a), isEqual("汇通快递", a1),
            isEqual("123", a2), isEqual("a", a3), isEqual("是", a4), isEqual(r,
                    a5), isEqual("常青店", a6), isEqual("是", a7));
...
function test170370() {
    ...
    var r = "anewCustomer" + getTimestamp(5);
    tapMenu("往来管理", "新增客户+");
    var keys = { "名称" : r, "允许退货" : "是", "拿货折扣" : "0.65" };
    var fields = editCustomerFields(keys);
    setTFieldsValue(getScrollView(), fields);
    tapButton(window, SAVE);
    delay();
...
function test170371() {
    ...
    var r = "anewCustomer" + getTimestamp(5);
    tapMenu("往来管理", "新增客户+");
    var keys = { "名称" : r, "允许退货" : "是", "拿货折扣" : "0.75" };
    var fields = editCustomerFields(keys);
    setTFieldsValue(getScrollView(), fields);
    tapButton(window, SAVE);
    delay();
...
function test170372() {
    ...
    var r = "anewCustomer" + getTimestamp(5);
    tapMenu("往来管理", "新增客户+");
    var keys = { "名称" : r, "允许退货" : "是", "拿货折扣" : "0.95" };
    var fields = editCustomerFields(keys);
    setTFieldsValue(getScrollView(), fields);
    tapButton(window, SAVE);
    delay();
...
function test170380() {
    ...
    var ret = isAnd(isEqual("234", qr1.data[0]["金额"]), isEqual("代收收款",
...
function test170437() {
    ...
    var kuc = add(add(add(qr.data[0]["库存"], qr.data[0]["在途数"]), add(
            qr.data[1]["库存"], qr.data[1]["在途数"])), add(qr.data[2]["库存"],
            qr.data[2]["在途数"]));
    ...
    var kuc1 = add(add(add(qr.data[0]["库存"], qr.data[0]["在途数"]), add(
            qr.data[1]["库存"], qr.data[1]["在途数"])), add(qr.data[2]["库存"],
            qr.data[2]["在途数"]));
    ...
    // qr = getQR();
    //
    // var kuc2 = add(add(add(qr.data[0]["库存"], qr.data[0]["在途数"]), add(
    // qr.data[1]["库存"], qr.data[1]["在途数"])), add(qr.data[2]["库存"],
    // qr.data[2]["在途数"]));
    ...
    // qr = getQR();
    //
    // ret = isAnd(ret, isEqual(kc, qr.data[0]["库存"]));
    ...
    // qr = getQR();
    //
    // ret1 = isAnd(ret1, isEqual(kuc2, qr.data[0]["库存"]));
...
function test170502() {
    ...
    tapMenu("销售开单", "按汇总", "按客户未结");
    var keys = { "日期从" : "2015-1-1", "到" : getToday(), "客户" : "xjkh1" };
    var fields = salesCustomerOutstandingFields(keys);
    query(fields);
    var qr = getQR();
...
function test170554() {
    ...
    var ret = isEqual("上级客户1", qr.data[0]["名称"]);
    ...
    tapMenu("销售开单", "按汇总", "按客户销售");
    var keys = { "日期从" : "2015-01-01", "到" : getToday(), "客户" : "xjkh1" };
    var fields = salesCustomerConsumeFields(keys);
    query(fields);
    var qr2 = getQR();
...
//        qr2 = getQR();
//        var kh = qr2.data[i]["客户"];
//        if (kh = "上级客户1") {
//            ret2 = false;
//        }
//    }
    
    var ret2 = isEqual("下级客户1", qr2.data[0]["名称"]);
    ...
    logDebug("len3=" + len3 + ", len1=" + len1 + ", ret=" + ret + ", ret1="
            + ret1 + ", ret2=" + ret2 + ", ret3=" + ret3);
    return ret && ret1 && ret2 && ret3;
}