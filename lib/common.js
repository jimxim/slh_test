...
function debugArray(a1) {
	...
	for (var i = 0; i < a1.length; i++) {
		...
		try {
			 ...
			 * try { msg += " label=" + a1[i].label(); } catch (e) { } try { msg += "
			 * value=" + a1[i].value(); } catch (e) { }
			 */
...
function debugElemnetsAt(view1) {
	if (view1) {
...
function add(menu1, menu2, hasSavePrompt, elements) {
	...
	for (var i = 0; i < elements.length; i++) {
		...
		switch (e1.type) {
		case "tf":
			view1.textFields()[e1.index].setValue(e1.value);
			break;
		case "sc":
			tapSingleChoice(view1, e1);
			break;
		case "mc":
			tapMultiChoice(view1, e1);
			break;
		default:
			UIALogger.logWarn("未支持的类型＝" + e1.type);
		}
...
// 保存时提示，点确定
function savePrompt() {
	tap(window.popover().actionSheet().collectionViews()[0].cells()[OK]
			.buttons()[OK]);
}
...
function exists(menu1, menu2, queryField) {
	...
	if (queryField) {
		...
		myDelay();
		var texts = getStaticTextsAtView();
...
function setTextFieldValueAtWindow(i, value) {
	setTextFieldValueAt(window, i, value);
}
function setTextFieldValueAtView(i, value) {
	setTextFieldValueAt(window.scrollViews()[0], i, value);
}
function setTextFieldValueAt(view1, i, value) {
	if (value) {
		view1.textFields()[i].textFields()[0].setValue(value);
	}
}

// 获取静态文本数组
function getStaticTextsAtView() {
	var view1 = window.scrollViews()[0];
