...
function testSalesNoColorSizeAll() {
    ...
    // if (cannotmodifyafterprint_0Params()){
...
function test170055() {
    ...
    var ret = isAnd(isEqual("欠款", value), isEqual("还款", value1), isEqual(
            "1500", k1), isEqual("1500", k2), isEqual("1801",
...
function test170061() {
    ...
    var ret = isAnd(isEqual("余款", value), isEqual("还款", value1), isEqual(
            getTextFieldValue(window, 2), getTextFieldValue(window, 6)),
            isEqual(getTextFieldValue(window, 2), getTextFieldValue(window, 3)));
...
function test170475() {
    ...
    // editSalesBillNoColorSize(json);
    //
    // var json = { "客户" : r, "核销" : "[5]", "onlytest" : "yes", "不返回" : "yes" };
    // editSalesBillNoColorSize(json);
    //
    // var json = { "客户" : r, "onlytest" : "yes" };
    // editSalesBillNoColorSize(json);
...
function testCs170065_2() {
    ...
    var ret = isAnd(isIn(getTextFieldValue(getScrollView(), 0), "3035"),
            isEqual("1", getTextFieldValue(getScrollView(), 3)), isIn(
                    getTextFieldValue(getScrollView(), 7), "3035"), isEqual(
                    "1", getTextFieldValue(getScrollView(), 10)), isIn(
                    getTextFieldValue(getScrollView(), 14), "3035"), isEqual(
...
function testCs170065_3() {
    ...
    tapMenu("销售开单", "开  单+");
    var json = { "明细" : [ { "货品" : "3035", "数量" : "2" } ], "现金" : "6000" };
    editSalesBillColorSize(json);
...
function test170085() {
    ...
    var k2 = getTextFieldValue(window, 2);
    var k3 = getTextFieldValue(window, 3);
    var ret = isAnd(isEqual("245", k2), isEqual("245", k3));
    ...
    query();
    var qr = getQR();
    var ret1 = isAnd(isEqual("245", qr.data[0]["金额"]), isEqual("245",
            qr.data[0]["现金"]), isEqual("0", qr.data[0]["未结"]));
...
function test170094() {
    ...
    var ydh = json["代收"]["运单号"];
    var bz = json["代收"]["备注"];
    var ret = isAnd(isEqual("416", money), isEqual("圆通速递", wls), isEqual("123",
            ydh), isEqual("a", bz));
...
function test170119() {
    ...
    tapMenu("销售开单", "开  单+");
    var json = { "客户" : "ls", "店员" : "000",
        "明细" : [ { "货品" : "3035", "数量" : "1" } ], "发货" : "仓库店", "明细输入框个数" : 8 };
    editSalesBillNoColorSize(json);
...
function test170191() {
    ...
    var json = { "客户" : "xw", "店员" : "000",
        "明细" : [ { "货品" : "3035", "数量" : "1.11" } ], "onlytest" : "yes" };
    editSalesBillNoColorSize(json);

    var a = getTextFieldValue(window, 12);
    var ret = isEqual("111", a);
...
function test170228() {
    // 设置产品折扣+单价小数位精确到厘
    tapMenu("货品管理", "新增货品+");
    var r = "lli" + getTimestamp(8);
    var keys = [ "款号", "名称", "品牌", "进货价", "产品折扣" ];
    var fields = editGoodsFields(keys, false, 0, 0);
    changeTFieldValue(fields["款号"], r);
    changeTFieldValue(fields["名称"], r);
    changeTFieldValue(fields["产品折扣"], "0.888");
    setTFieldsValue(getScrollView(), fields);
    saveAndAlertOk();
    tapPrompt();
    delay();
    ...
    var ret = isAnd(isEqual(r, getTextFieldValue(getScrollView(), 0)), isEqual(
            r, getTextFieldValue(getScrollView(), 1)), isEqual("0.888",
            getTextFieldValue(getScrollView(), 13)));
    tapButton(window, RETURN);

    return ret;
}
function test170231() {
    // 折扣三位小数+单价小数位精确到角时销售开单
    tapMenu("货品管理", "新增货品+");
    var r = "lli" + getTimestamp(8);
    var keys = [ "款号", "名称", "品牌", "进货价", "产品折扣" ];
    var fields = editGoodsFields(keys, false, 0, 0);
