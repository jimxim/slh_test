...
function editSalesBillGetValue() {
    ...
    var cardTFindex = getCardTFindex();
    var totalNumTFindex = getTotalNumTFindex();
    var arr = {};
    arr["结余"] = getStaticTextValue(window, 1);
    arr["现金"] = getStaticTextValue(window, 2);
    arr["应"] = getStaticTextValue(window, getEditSalesTFindex("客户,厂商", "应"));
    arr["实"] = getStaticTextValue(window, getEditSalesTFindex("客户,厂商", "实"));

    arr["核销"] = getStaticTextValue(window, cardTFindex - 1);
    arr["刷卡"] = getStaticTextValue(window, cardTFindex);
    arr["代收"] = getStaticTextValue(window, cardTFindex + 1);
    arr["实付"] = getStaticTextValue(window, cardTFindex + 1);

    arr["总数"] = getStaticTextValue(window, totalNumTFindex);
    arr["总计"] = getStaticTextValue(window, totalNumTFindex + 1);
    arr["汇款"] = getStaticTextValue(window, totalNumTFindex + 2);
