...
function testPurchase001() {
    ...
    run("【采购入库】条件查询，清除按钮", "test120001_2");
...
function testPurchase002() {
    ...
    run("【采购入库-按批次查】按批次查->作废、挂单操作和查询", "test120044_120052");
...
function test120001_2() {
    ...
    query(fields);
    qr = getQR();
    var ret = isAnd(isEqual(batch + 1, qr.data[0]["批次"]), isEqual("Rt",
            qr.data[0]["厂商"]));
...
function test120003() {
    ...
    qr = getQR();
    ret = isAnd(ret, isEqual(a + 1000, qr.data[0]["余额"]));
...
function test120007() {
    ...
    tapMenu("采购入库", "按批次查");
    keys = { "日期从" : getToday(), "日期到" : getToday(), "作废挂单" : "正常" };
    fields = purchaseQueryBatchFields(keys);
    query(fields);
    qr = getQR();
...
function test120007_1() {
    ...
    query(fields);
    var qr = getQR();
    var ret = isAnd(isEqual(getToday("yy"), qr.data[0]["日期"]), isEqual(getDay(
            -1, "yy"), qr.data[1]["日期"]), isEqual("2", qr.total));
...
function test120008_2() {
    ...
    ret = isAnd(ret, isEqual(getToday(), getTextFieldValue(window, 0)),
            isEqual(getToday(), getTextFieldValue(window, 1)), isEqual("",
                    ...
                    getTextFieldValue(window, 5)), isEqual("",
...
function test120010() {
    ...
    var expected = { "序号" : 0, "名称" : "Vell", "现金" : 600, "刷卡" : 400,
...
function test120010_1() {
    ...
    ret = isAnd(ret, isEqual("100", sub(sm1, sum1)), isEqual("100", sub(sm2,
...
function test120011_2() {
    ...
    var ret = isAnd(isEqual(sum1, qr.counts["金额"]), isEqual(sum2,
            qr.counts["总数"]));
    return ret;
}
...
function test120031_120032() {
    ...
    var ret = isEqual(actual, qr.counts["数量"]);
...
function test120044_120052() {
    ...
    query(fields);
    qr = getQR();
    var ret = isEqualObject(a, qr.data[0]);
...
function test120017() {
    ...
    query();
    qr = getQR();
    expected = { "批次" : batch + 1, "日期" : getToday(""), "厂商" : "Vell",
        "店员" : "总经理", "总数" : "50", "金额" : "6400", "现金" : "6400", "刷卡" : "0",
        "汇款" : "0", "操作人" : "总经理" };
    ret = isAnd(ret, isEqualQRData1Object(qr, expected));
...
function test120018() {
    ...
    query();
    qr = getQR();
    var ret = isAnd(isEqual(batch + 1, qr.data[0]["批次"]), isEqual(getToday(""),
            qr.data[0]["日期"]), isEqual("Rt", qr.data[0]["厂商"]), isEqual("35",
            qr.data[0]["总数"]), isEqual("4700", qr.data[0]["金额"]), isEqual(
            "100", qr.data[0]["现金"]), isEqual("600", qr.data[0]["刷卡"]),
            isEqual("4000", qr.data[0]["汇款"]));

    tapFirstText();
    ret = isAnd(ret, isEqual("Rt", getTextFieldValue(window, 0)), isEqual(
            "004,店长", getTextFieldValue(window, 4)), isEqual("3035,jkk",
