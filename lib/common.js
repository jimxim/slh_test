        ...
        for (var i = 0; i <= curPageTotal; i++) {
            ...
            for (var j = 0; j < titleTotal; j++) {
                ...
                if (index < texts.length) {
                    ...
                    var v1 = text.value();
                    var x1 = text.rect().origin.x;
                    var t = getKeyByXy(oTitle, x1);
                    if (isDefined(t)) {
                        data1[t] = v1;
                    } else {
                        logDebug(v1 + " 没找到对应标题" + ",x1=" + x1); // 选择列跳过
                    }
                } else {
                    hasError = true;
                }
...
function sortByTitle(title, isNum) {
    ...
    var ret1 = compareQR(title, isNum);

    tapTitle(getScrollView(), title); // 再点击一下后是降序
    var ret2 = compareQR(title, isNum, "desc");
