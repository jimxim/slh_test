...
function editSalesBillUnpay(o) {
    ...
    var v = o[key];
    if (isDefined(v)) {
        var btns = getButtons(window);
        for(var i=0,length=btns.length;i<length;i++){
            var name=btns[i].name();
            if(name=="未付"||name=="欠"){
                btns[i].isVisible()?btns[i].tap():btns[i+1].tap();//2个未付按钮重合，第二个才能触发
                break;
            }
        }
    }
...
function getDetSizheadTitle(order) {
    ...
    for (var j = qrTitle1.index + 1; j < qrTitle2.index; j++) {
        ...
        if (width > 10) {// 忽略隐藏标题
          ...
          logDebug('title='+title+' idx='+j+' width='+width+' 跳过');
        }
    }
    ...
    // }
    // }
    debugObject(ret, "getDetSizheadTitle ret");
    return ret;
}
