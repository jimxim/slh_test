...
function editSalesBillDetColorSize(o) {
    var goodsFieldIndex = -2, detLine = 0;
...
function editSalesBillGetValue(o) {
    ...
    index = getValueFromCacheF1("getStoreTFindex");
    if (index >= 0) {
...
function editSalesBillSave(o) {
    ...
    o["输入框值"] = editSalesBillGetValue(o);// gCache
    o["明细值"] = getQRDet(getScrollView(-1), o);// gCache
...
function editSalesBillAddCustomer(o) {
    ...
    n = o["店员"];
    if (isDefined(n)) {
        fields.push(new TField("店员", TF, 2, n, -1, 0));// 不稳定，有时店员输入不了
    }
    n = o["适用价格"];
    if (isDefined(n)) {
