...
UIATarget.onAlert = function onAlert(alert) {
	...
	if(debug) {
		UIALogger.logMessage("alertName="+alertName+" alertMsg="+alertMsg+" alert="+alert);
	}
...
function indexOfTextsByTitle(a, texts, title) {
	...
	for(var i=0; i<texts.length; i++ ) {
		...
		if( title == texts[i].value() ) {
			...
			if(title1IndexPre>0 && titleIndexPre>0 && title1Index-title1IndexPre == titleIndex-titleIndexPre && 
				title1IndexNext < texts.length && title1!=texts[title1IndexNext].value()) {
				break;
			}
		}
	}
...
function tapSingleChoice(view1, obj) {
	...
	var value = obj.value;
	tap( window.popover().scrollViews()[0].staticTexts()[value] );
...
function tapMultiChoice(view1, obj) {
	...
	for(var i=0; i<valueBtnIndexes.length; i++) {
		tap(window.popover().scrollViews()[0].buttons()[valueBtnIndexes[i]]);
	}
	
	tap(window.popover().buttons()[OK]);
}

//通用增加功能
function add(menu1, menu2, hasSavePrompt, textFields, singleChoices, multiChoices) {
	tapMenu(menu1,menu2);
	var view1 = window.scrollViews()[0];
	for(var i=0;i<textFields.length; i++) {
		var tf = textFields[i];
		view1.textFields()[tf.index].setValue(tf.value);  
	}
	...
	myDelay();
	tap(window.buttons()[RETURN]);
}

function savePrompt() {
	tap(window.popover().actionSheet().collectionViews()[0].cells()[OK].buttons()[OK]);
}

function saveAtWindow() {
	tap(window.buttons()[SAVE]);
}
...
function exists(menu1, menu2, queryField) {
	...
	var ret = 0; 
	if(queryField) {
		var queryFieldIndex = queryField.index;
...
function setTextFieldValueAtWindow(i, value) {
	if(value) {
		window.textFields()[i].textFields()[0].setValue(value);
	}
}
function setTextFieldValueAtView(i, value) {
	if(value) {
		window.scrollViews()[0].textFields()[i].textFields()[0].setValue(value);
	}
}
