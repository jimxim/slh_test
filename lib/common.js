...
function getQResultTitle(texts, title,titleTotal) {
	...
	logDebug("titleIndex=" + titleIndex+" titleIndexPre="+titleIndexPre+ " titleTotal="+titleTotal);
...
// 点击标题排序，按升序比较值，返回真或假
function sortByTitle(title, titleIndex) {
	tapTitle(getScrollView(), title); // 点击一下后是升序

	var ret = true;
	var qr = getQResult();
	for (var i = 0; i < qr.curPageTotal; i++) {
		var name = qr.data[i][titleIndex];
		if (i > 0) {
			var namePre = qr.data[i - 1][titleIndex];
