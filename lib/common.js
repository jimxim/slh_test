...
function getQRtable1(view1, offset) {
    ...
    var table1 = getTableView(view1, -1);
 ...
 * @returns {Boolean}
 */
function isNoEqual(expected, actual) {
    var ret = expected != actual;
    logDebug("expected=" + expected + ",actual=" + actual + ",Noequal=" + ret);
    return ret;
}
/**
 * 对象是否相等
 ...
 * @returns {Boolean}
 */
function sortByTitle(title, isNum,pageInfoView, dataView, firstTitle, titleTotal) {
    var t1 = getTimestamp();
    tapTitle(getScrollView(), title); // 点击一下后是升序
    var ret1 = compareQR(title, isNum,"asc",pageInfoView, dataView, firstTitle, titleTotal);

    tapTitle(getScrollView(), title); // 再点击一下后是降序
    var ret2 = compareQR(title, isNum, "desc",pageInfoView, dataView, firstTitle, titleTotal);
...
function add(n2, n1) {
    var ret = Number(n2) + Number(n1);
    logDebug("add n2=" + n2 + " n1=" + n1 + " ret=" + ret);
    return ret;
}
/**
 * 两个对象做减法,jo2-jo1
