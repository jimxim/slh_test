...
function testSalesNoColorSize001() {
    ...
    run("【销售开单－开单】整单折扣下检查折扣3位小数", "test170497_170086");
    ...
    run("【销售开单-开单-加工货品】没有权限看价格的店员采购加工货品", "test170430");
...
function test170061() {
    tapMenu("销售开单", "开  单+");
    var json = { "客户" : "xjkh1", "特殊货品" : { "抹零" : 9, "打包费" : 10 },
        "onlytest" : "yes" };
    editSalesBillNoColorSize(json);
    ...
    var ret1 = isEqual(3, sub(len, len1));
...
function test170076_2() {
    ...
    var f4 = new TField("单价", TF, 4, "300.18888");
    var fields = [ f4 ];
    setTFieldsValue(getScrollView(), fields);
...
function test170076_3() {
    ...
    var f4 = new TField("单价", TF, 4, "300.188888");
    var fields = [ f4 ];
    setTFieldsValue(getScrollView(), fields);
...
function test170080_170084() {
    ...
    var ret1 = isEqual(r1, getTextFieldValue(getScrollView(), 5));
    ...
    var ret4 = isEqual(r1, getTextFieldValue(getScrollView(), 5));
    ...
    var ret5 = isEqual(r1, getTextFieldValue(getScrollView(), 7));
...
function test170118() {
    ...
    debugArray(alertMsgs);
    var alertMsg1 = getArray1(alertMsgs, -1);
    var ret1 = isAnd(isIn(alertMsg1, "操作失败，[操作失败，门店[常青店]中[" + r1 + "," + r1
            + ",均色,均码] 库存不足，当前库存数为[0.00]!] "));
...
function test170139_1() {
    ...
    var ret = isAnd(isIn(k0, "3035"), isEqual("均色", k1), isEqual("均码", k2),
            isEqual("2", k3), isEqual("李四", f0), isIn(f5, "总经理"));
    ...
    var ret1 = isAnd(isIn(getTextFieldValue(getScrollView(), 0), "3035"),
            isEqual("均色", getTextFieldValue(getScrollView(), 1)), isEqual("均码",
                    getTextFieldValue(getScrollView(), 2)), isEqual("2",
                    getTextFieldValue(getScrollView(), 3)), isEqual("mxbz",
                    getTextFieldValue(getScrollView(), 7)), isEqual("李四", f0),
            isIn(f5, "总经理"), isEqual(getToday(), getTextFieldValue(window, 8)),
            isEqual("zdbz", getTextFieldValue(window, 9)), isEqual("0.588",
...
function test170139_2() {
    ...
    var ret4 = isAnd(isIn(getTextFieldValue(getScrollView(), 0), r), isEqual(
            ...
            getTextFieldValue(getScrollView(), 3)), isEqual(r1,
...
function test170176() {
    ...
    tapFirstText();
    var ret1 = isAnd(isEqual(money, getTextFieldValue(window, 8)), isEqual(
...
function test170207() {
    ...
    var ret2 = isAnd(isEqual(je, getTextFieldValue(window, 3)), isEqual(
            ...
            getTextFieldValue(getScrollView(), 17)), isEqual("李四", kh),
            isEqual(3, sl), isAqualOptime(getOpTime(), opt));
...
function test170227() {
    ...
    tapFirstText();
    var ret = isAnd(ret, isEqual("3035,jkk", getTextFieldValue(getScrollView(),
            0)), isEqual(-6, getTextFieldValue(getScrollView(), 3)), isEqual(
            je, getTextFieldValue(window, 3)), isEqual("李响", kh), isEqual(-6,
...
function test170497_170086() {
    ...
    var k2 = getTextFieldValue(window, 2);
    var k3 = getTextFieldValue(window, 3);
    ret = ret && isAnd(isEqual("688", k2), isEqual("688", k3));
    ...
    query();
    var qr = getQR();
    var ret2 = isAnd(isEqual("688", qr.data[0]["金额"]), isEqual("688",
            qr.data[0]["现金"]), isEqual("0", qr.data[0]["未结"]));
...
function test170522() {
    ...
    var ret1 = isAnd(isEqual(md, "常青店"), isEqual(pc, add(pc1, 1)), isEqual(rq,
