...
function isEqual(expected, actual) {
	...
	logDebug("expected=|" + expected + "| actual=|" + actual + "| ret=" + ret);
	return ret;
}

// 如果 b 包含 a 返回真
function isIn(b, a) {
	var ret = b.indexOf(a) != -1
...
// 设置文本框值
function setTextFieldValue(view1, e1) {
	var i = e1.index;
	var value = e1.value;
...
// 设置文本块
function setTextViewValue(view1, e1) {
	var i = e1.index;
	var value = e1.value;
...
function setTFieldsValue(view1, elements) {
	...
	for ( var i in elements) {
		...
		case TF:
			...
			break;
		case TF_AC:
			setTextFieldACValue(view1, e1);
			break;
		case TF_SC:
			setTextFieldSCValue(view1, e1);
			break;
		case TF_DT: // 日期
			setTextFieldDtValue(view1, e1);
			break;
		case TV:
			setTextViewValue(view1, e1);
			break;
		case BTN:
			break;
		case BTN_SC:
...
function checkShowFields(view1, fields) {
	...
	for ( var i in fields) {
		...
		switch (type2) {
		case TF:
			actual = getTextFieldValue(view1, i);
			break;
		case TV:
			actual = getTextViewValue(view1, i);
			break;
		default:
