...
function compareQR(title, type, order, pageInfoView, dataView, firstTitle,
    ...
    if (isDefined(type)) {
        ...
        for (var i = 0; i < qr.curPageTotal; i++) {
            ...
            case IS_DATE2:
                ...
                break;
            case IS_OPTIME:
                value = getDayToFullYear(value);// 日期补全，防止跨年验证错误
                break;
            default:
                logInfo("未知type=" + type);
            }
    ...
    logDebug(gMenu1 + "-" + gMenu2 + "," + title + "," + type + "," + order
            + ",ret=" + ret + "," + getTakeTimeMsg(t1));
    return ret;
}
 ...
 * @param title
 * @param firstTitle
 * @param order 寻找顺序 默认0开始  desc倒序查找
 * @returns {QResultTitle}
 */
function getQResultTitle(texts, title, firstTitle, order) {
    if (isUndefined(firstTitle)) {
        firstTitle = TITLE_SEQ;
    }
    var firstTitleIdx = 0, lastTitleIdx = 0, titleIndex = 0, y = 0, yPre = 0, i;
    if (isDefined(order) && order == "desc") {
        for (i = texts.length - 1; i >= 0; i--) {
            if (firstTitle == texts[i].value()) {
                firstTitleIdx = i;// 起始标题下标，找不到就默认0
                break;
            }
        }
    } else {
    ...
    for (i = firstTitleIdx; i < texts.length; i++) {
        yPre = y;
        y = getY(texts[i]);
        if ((yPre > 0 && !isAqualNum(y, yPre, 10))) {
            break; // Y轴可能有偏移量，大概为5，第二行跳出
        } else {
            lastTitleIdx = i;
        }
        if (title == texts[i].value()) {
            titleIndex = i // 在静态文本数组中的下标，开单界面标题取值用
        }
    }
    var titleTotal = lastTitleIdx - firstTitleIdx + 1; // 显示标题总个数

    logDebug("title=" + title + " titleIndex=" + titleIndex + " titleTotal=" +
      titleTotal + " firstTitleIdx=" + firstTitleIdx + " yPre=" + yPre + " y=" + y +
...
function getFirstIndexOfTextsByTitle(texts, title, firstTitle) {
    var qrTitle = getQResultTitle(texts, title, firstTitle);
...
function setAreaValue(view1, f) {
    try {
        var tableViewIndex = f.p1 ? f.p1 : 0;
        var view2 = getTableView(getPop(),tableViewIndex);
        ...
        var btn = getButton(cell,0);//7.2731 控件改变
        tap(btn);
        delay();
    } catch (e) {
        getPop().dismiss();
 ...
 * @param f
 */
function setMultiChoiceValue(view, f) {
    var btnIndex = f.index;
    tapButton(view, btnIndex);
...
function setSingleChoiceValue(view1, f) {
    delay();
    var btnIndex = f.index;
    tap(view1.buttons()[btnIndex], true);
    delay();// 为了让弹出加载完全
    var value = f.value, p1 = f.p1, p2 = f.p2, p3 = f.p3;
    if (isDefined(p3) && p3 == "collectionView") {
        var obj = getPopCollectionView(window, p2).cells()[value];
        tap(obj, true);
        delay();
        return;
    }
...
function setTextFieldSCValue(view1, f) {
    ...
    delay(0.5)// 等待popView出现，新增收支中偶尔pop取不到 160927
    ...
    delay(0.5);
    tapKeyboardHide();// 收键盘 防止挡住界面 161213
}
 ...
 * @param f
 */
function setTextFieldTypeValue(view1,f) {
  var i = f.index,value=f.value;
  tap(view1.textFields()[i].textFields()[0]);
  delay(0.5);
//  window.popover().tableViews()[0].cells()[value].buttons()[0].tap();//单选，简单处理
  setAreaValue(view1, f);
}
/**
 * 新增货品界面类别
 ...
 * @param f
 */
function setPickerWheelValue(view1,f){
    var idx = f.index,value=f.value.split("-");
    tapButton(view1,idx);
    delay(0.5);
    var pop=getPop(window,-1);
    var picker=pop.pickers()[0];
    for (var j = 0; j < value.length; j++) {
      for(var i=0;i<10;i++){//常用的就在前10个
          var wheel=picker.wheels()[j];
          var name=wheel.name();
          if(name.indexOf(value[i])!=-1){
            ...
            break;
              ...
              if(i==9){
                  tapButton(pop,'取 消');
                  logWarn('前10个显示的类别中未找到名称为 '+value+' 的类别');
                  break;
              }
              wheel.tapWithOptions({tapOffset:{x:0.56, y:0.64}});
              delay(0.5);// 防止取wheel错误
          }
      }
    }
}
...
function setTextFieldValueByKeyboard(tf, value) {
    ...
    tf.setValue("");
    delay(0.5);// 等待键盘弹出，防止取不到导致的漏输问题 16-10-08
    var kb = app.keyboard();
...
function tapMenu1(menu_name) {
    ...
    // 防止异常情况各种开单界面没有返回的情况
    var btn = getButton(window, RETURN);// 这里差不多有1S的时间，规避了界面延迟空白的问题 16-10-08
    if (isUIAButton(btn) && oldMenu1 != gMenu1) {
...
function tapMenu2(menu_name) {
    ...
    gMenu2 = menu;
    tap(window.buttons()[menu]);
    if (gMenu2.indexOf("+") != -1) {
        delay(0.5);// 开单界面加载较慢，多等待一会儿
    }
    delay(0.5);
    // gMenu2 = menu_name;
...
function tapMenu3(menu_name) {
    ...
    var pop = getPop();
    try {
        return tapButton(pop, menu_name);
    } catch (e) {
        if (!isUIAElementNil(pop)) {
            pop.dismiss();
        }
 ...
 * @param firstTitle
 */
function tapTitle(view1, title, firstTitle) {
