 ...
 * @param o 输入对象，eg: { "客户" : "zhhz1", "店员" : "005,","日期" : "2015-10-01",
 *            "价格":"打包价","明细" : [ { "货品" : "k300", "数量" : 5,"备注" : "xx" } ],
 *            "核销" : [ 5 ]，"特殊货品" : { "抹零" : 9, "打包费" : 10 } , "现金" :
...
function editSalesBillAgency2(obj) {
    ...
    v = obj[key];
    if (isDefined(v)) {
        f = new TField(key, TF_AC, -4, v, -1, 0);
        fields.push(f);
    }
    key = "运单号";
    v = obj[key];
    if (isDefined(v)) {
        f = new TField(key, TF, -3, v);
        fields.push(f);
    }
    key = "备注";
    v = obj[key];
    if (isDefined(v)) {
        f = new TField(key, TF, -2, v);
        fields.push(f);
    }
    key = "代收金额";
    v = obj[key];
    if (isDefined(v)) {
        f = new TField(key, TF, -1, v);
        fields.push(f);
    }
    ...
    obj[key] = getTextFieldValue(window, -1);
