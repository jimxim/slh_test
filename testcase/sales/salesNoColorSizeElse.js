...
function testSalesNoColorSizeElse001() {
    ...
    // run("【销售开单】按挂单--正常功能检查", "test170303_2");
...
function testSalesNoColorSizeElseAll() {
    ...
    // run("【【销售开单－按批次查】作废挂单 选择除“正常”以外其它三个条件时，查看IPAD端屏幕底部的汇总数据",
    // "test170007");//
    ...
    // run("【销售开单－销售汇总-按退货汇总】排序/翻页/快速翻页", "test170321_170322_170323");
    // run("【销售开单－销售汇总-按客户销售】按客户销售", "test170325");
    // run("【销售开单－销售汇总-按客户销售】清除功能", "test170326");
    // run("【销售开单－销售汇总-按退货汇总】排序/翻页/快速翻页", "test170327_170328_170329_170330");
    ...
    // run("【销售开单－销售汇总-按退货汇总】排序/翻页/快速翻页", "test170341_70342_170343_170346");
    // run("【销售开单－销售汇总-按客户上货】点击查询记录后页面检查", "test170344");//
    // run("【销售开单－销售汇总-按客户上货】拿货次数检查", "test170345");
    // run("【销售开单－销售汇总-客户对帐单】清除功能", "test170351");
    // run("【销售开单－销售汇总】按类别汇总", "test170356");
    // run("【销售开单－销售汇总-按类别汇总】清除功能", "test170357");
    ...
    // run("【销售开单】代收收款作废功能", "test170398");
    // run("【销售开单】代收收款界面门店查询条件检查", "test170399");
    // run("【销售开单】挂单界面打印时提示检查", "test1703400");
    // run("【销售开单】单价为3位小数时，连续开单，检查价格", "test170402");
    // run("【销售开单-按订货开单】按订货开单，默认日期检查", "test170403");
    // run("【销售开单】均色均码模式下，开单输入款号之后的款号框不能修改", "test170406");
    // run("【销售开单】按订货开单界面款号查询结果检查", "test170409");
    // run("【销售开单-更多】检查代收收款金额", "test170410");
    // run("【销售开单】价格3位小数情况下检查小计值", "test170411");
    // run("【销售开单】销售价格允许改高不允许改低--价格改低", "test170414");
}
...
function test170006() {
    ...
    var a8 = qr.data[0]["配货"];
    var ret = isAnd(isEqual(getToday(""), a1), isEqual("常青店", a2), isEqual(
            "李四", a3), isEqual("总经理", a4), isEqual("250", a5),
...
function test170008() {
    ...
    var ret = isAnd(isEqual(getToday(""), a1), isEqual("常青店", a2), isEqual(
            "李响", a3), isEqual("总经理", a4), isEqual("250", a5),
            isEqual("1", a6), isEqual("0", a7), isEqual("否", a8), isEqual(opt,
                    a10));
    logDebug("ret=" + ret);
    return ret;
}
...
function test170255_170256_170257() {
    ...
    var ret1 = ret && sortByTitle("差异数", IS_NUM);
...
function test170280_170281_170282() {
    ...
    var ret1 = ret && sortByTitle("运单号", IS_NUM);
...
function test170303() {
    ...
    var json = {
        ...
        "特殊货品" : { "抹零" : 100, "打包费" : 100 }, "onlytest" : "yes" };
    editSalesBillNoColorSize(json);
    ...
    var ret = isAnd(isEqual("李四", a), isEqual(getToday(""), b), isEqual("666",
            c));

    logDebug(" ret" + ret);
    return ret;
}
function test170303_1() {
    tapMenu("销售开单", "开  单+");
    var json = {
        "客户" : "ls",
        "店员" : "000",
...
function test170303_2() {
    // tapMenu("销售开单", "开 单+");
    // var json = {
    // "客户" : "ls",
    // "店员" : "000",
    // "明细" : [ { "货品" : "3035", "数量" : "1" }, { "货品" : "8989", "数量" : "1" } ],
    // "特殊货品" : { "抹零" : 100, "打包费" : 100 }, "备注" : "zdbz", "onlytest" : "yes"
    // };
    // editSalesBillNoColorSize(json);
    //
    // tapButtonAndAlert("挂 单", OK);
    ...
    var a10 = qr.data[0]["操作日期"];
    var ret1 = isAnd(isEqual(getToday(""), a1), isEqual("常青店", a2), isEqual(
            "李四", a3), isEqual("总经理", a4), isEqual("666", a5),
...
function test170307() {
    ...
    tapMenu("销售开单", "按汇总", "按金额汇总");
    var keys = { "日期从" : getDay(-10), "到" : getToday() };
    ...
    var ret = isAnd(isEqual(getDay(-10), a), isEqual(getToday(), a1));
...
function test170307_1() {
    ...
    tapMenu("销售开单", "按汇总", "按金额汇总");
    var keys = { "日期从" : getDay(-1), "到" : getToday() };
    var fields = salesPriceFields(keys);
    setTFieldsValue(window, fields);
    query(fields);
    // 点击翻页
...
//    logDebug("ret=" + ret);
    ...
    var ret1 = isAnd(isEqual(qr.counts["现金"], sum1), isEqual(qr.counts["刷卡"],
            sum2), isEqual(qr.counts["汇款"], sum3), isEqual(qr.counts["代收"],sum4));
...
function test170308() {
    ...
    keys = { "发生日期从" : getDay(-10), "日期到" : getToday(), "门店" : "常青店",
        "款号" : "k300" };
    ...
    var ret = isAnd(isEqual("铅笔裤", a), isEqual("15-10-13", a1), isEqual("均色",
...
function test170309() {
    ...
    tapMenu("销售开单", "按汇总", "按款号汇总");
    var keys = { "发生日期从" : getDay(-10), "日期到" : getToday(),
        "上架日期从" : "15-10-13", "上架日期到" : "15-10-13", "款号" : "k300", "颜色" : "均色",
        "尺码" : "均码" };
    var fields = salesCodeFields(keys);
    query(fields);
    ...
    var ret = isAnd(isEqual(getDay(-10), a), isEqual(getToday(), a1), isEqual(
            "", a2), isEqual("", a3), isEqual("", a4), isEqual("", a5),
            isEqual("", a6), isEqual("", a7));
...
function test170310_170311_170312() {
    ...
    tapMenu("销售开单", "按汇总", "按款号汇总");
    var keys = { "发生日期从" : getToday(), "日期到" : getToday() };
    var fields = salesCodeFields(keys);
    query(fields);
    var qr = getQR();
    scrollNextPage();
...
function test170315_170316_170317() {
    ...
    // ret = ret && sortByTitle("尺码");
    ret = ret && sortByTitle("数量", IS_NUM);
    // ret = ret && sortByTitle("退货均价", IS_NUM);//服务端错误
    ret = ret && sortByTitle("小计", IS_NUM);
...
function test170318() {
    ...
    var qr = getQR2(getScrollView(1), "店员", "小计");
    var a = qr.data[0]["客户"];
    var a1 = qr.data[0]["款号"];
    var a2 = qr.data[0]["名称"];
    var a3 = qr.data[0]["数量"];
...
function test170319() {
    ...
    var ret = isAnd(isEqual(qr.counts["现金"], sum1), isEqual(qr.counts["刷卡"],
            ...
            sum8), isEqual(qr.counts["实销额"], sum9), isEqual(qr.counts["特殊货品"],
            sum10), isEqual(qr.counts["实销额2"], sum11));
    logDebug("sum1=" + sum1 + "sum2=" + sum2 + "sum3=" + sum3 + "sum4=" + sum4
            + "sum5=" + sum5 + "sum6=" + sum6 + "sum7=" + sum7 + "sum8=" + sum8
            + "sum9=" + sum9 + "sum10=" + sum10 + "sum11=" + sum11);
...
function test170320() {
    ...
    var ret = isAnd(isEqual(getToday(), a), isEqual("", a1), isEqual("", a2),
            isEqual("", a3));
...
function test170321_170322_170323() {
    ...
    // ret = ret && sortByTitle("实销额2", IS_NUM);
...
function test170338() {
    ...
    var ret = isAnd(isEqual(qr.counts["实销数"], sum1));
    logDebug("sum1=" + sum1);
...
function test170340() {
    ...
    var a5 = getTextFieldValue(window, 5);
    var a6 = getTextFieldValue(window, 6);
    var ret = isAnd(isEqual("", a), isEqual("", a1), isEqual("", a2), isEqual(
...
function test170363() {
    ...
    var ret = isAnd(isEqual("", a), isEqual(getToday(), a1), isEqual("", a2));
...
function test170402() {
    ...
    var f4 = new TField("单价", TF, 4, "416.555");
    var fields = [ f4 ];
    setTFieldsValue(getScrollView(), fields);
...
function test170406() {
    ...
    var f = new TField("货品", TF, 0, "GG5,衣服");
    var fields = [ f ];
    setTFieldsValue(getScrollView(), fields);
...
function test170411() {
    ...
    var ret = isAnd(isEqual("410.307", a), isEqual("250", a1),
            isEqual("90", a2));
    tapButton(window, RETURN);
...
// function test170414() {
// 开启参数 销售价格允许改高不允许改低 为销售价不能低于零批价,常青店开单员：005
// }
