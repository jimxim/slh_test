...
function editSalesBillGetValue() {
    ...
    var arr = {};
    arr["客户"] = getTextFieldValue(window, cashTFindex - 2);

    arr["现金"] = getTextFieldValue(window, cashTFindex);
    arr["应"] = getTextFieldValue(window, cashTFindex + 1);// 有些地方显示为应收
    ...
    // 不同开单模式，显示的内容不同
    arr["代收/实付"] = getTextFieldValue(window, cardTFindex + 1);
    if (arr["代收/实付"] == "") {
        arr["代收/实付"] = 0;
    }
...
function editSalesBillAddCustomer(o) {
    ...
    n = o["适用价格"];
    if (isDefined(n)) {
        fields.push(new TField("适用价格", BTN_SC, 0, n));// 还需修改
    }
    n = o["地址"];
    if (isDefined(n)) {
