...
function tapFirstQRText(view1, qr) {
	var texts = getStaticTexts(view1);
	var i = getFirstIndexOfTextsByQR(texts, qr);
	tap(texts[i]);
}
// 保存时提示，点确定
...
function getFirstIndexOfTextsByQR(texts, qr) {
	var ret = qr.firstTitleIndexOfTexts + qr.total;
	if (ret >= texts.length) {
		ret = -1
	}
	return ret;
