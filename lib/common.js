 ...
 * @param p2 TF_AC时=cellIndex，TF_AREA时＝展开的组合,BTN_SC时可选的popview下标，比如-1
 ...
 * 点击标题排序，按升序比较值，返回真或假
 * @param title
 * @param isNum 是否数字，不输默认不是数字，要用常量 IS_NUM
 * @returns {Boolean}
 */
...
function tapPrompt(i) {
    ...
    var b1 = getPop().actionSheet().collectionViews()[0].cells()[i].buttons()[i];
    var ret = tap(b1);
    delay();
    return ret;
}
