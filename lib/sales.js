...
function editSalesBillDetSizehead(o) {
    ...
    if (details.length > 0) {
        ...
        tfNum = getSalesBillDetTfNum({});// 获取一行的输入框个数，可能有隐藏的，不能用标题个数判断
        // o["标题"] = o1;标题自取，传递会出错
    }
    var view1 = getScrollView(-1);
    for (var i = 0; i < details.length; i++) {
        ...
        if (isDefined(d["单价"])) {
            f = new TField("单价", TF, start + o1["单价"], d["单价"]);
            setTFieldsValue(view1, [ f ]);
        }
    }
    return o;
...
function getDetSizheadTitle() {
    ...
    var qrTitle1 = getSalesBillDetTitle1Index(texts);// 起始标题 # 图
    var qrTitle2 = getQResultTitle(texts, "操作", "#");// 结束标题 操作
    ...
    ret["明细输入框个数"] = qrTitle2.index - qrTitle1.index - 1;// 只是有效的输入框个数
    // debugObject(colX);
    // 其它尺码加入标题
