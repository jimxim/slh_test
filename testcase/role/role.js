...
function testBuyer003() {
    ...
    run("【采购入库-新增入库】入库门店检查", "ts120123");
    run("【采购入库-新增入库】复杂模式采购员做调拨和采购入库时需显示进货价", "ts120090");
}

function testShopkeeper004All() {
    // customer.js
    run("【往来管理-客户查询】客户查询->消费明细_权限验证", "test110002_1All");
    run("【往来管理】店长查看客户门店帐", "test110031_110032");
    run("【往来管理】店长查看按上级单位", "test110022_1");// 有bug，待测
    run("【往来管理-客户查询】非总经理角色修改有欠款或余款的客户的名称", "ts110057");
    ...
    run("【往来管理-更多-新增积分调整】店长只能新增本门店的积分调整", "ts110103");
    run("【往来管理-客户账款】店长权限", "ts110115");
    ...
    run("【货品管理-当前库存】门店库存是否允许跨门店查询", "ts100166");
    run("【当前库存-库存调整】店长只能调整本门店的库存", "ts100168");
    ...
    run("【货品管理-新增货品】非总经理只能录入自己门店的库存", "ts100183");// 新增界面门店下拉列表&&库存录入
...
// 仓库店店长登陆验证 绑定仓库为文一店
function test104() {
    run("【采购入库-批量入库】门店绑定仓库，批量入库的入库门店为开单门店", "ts120114");
}
...
// 常青店营业员006 用来验证仅本人创建权限的用例
function login006All() {
    run("【货品管理-当前库存】记录权限仅本人创建时，查看当前库存信息", "ts100158");// 含100165
}
// 常青店配货员007
function login007All() {
    run("【销售订货-新增订货】检查配货员是否能查看别的门店的销售订货单", "test160117");
    run("【销售订货-新增订货】配货员按订货开单后,单据属性门店内容检查", "test160118");
}
// 文一店配货员
function login407All() {
    run("【销售开单-开单】异地仓库+门店绑定仓库，配货员验证", "test170760");
}
// 敏感字段检查 有
