 ...
 * @returns {Boolean}
 */
function compareQR(title, isNum, order) {
    ...
    logDebug(title + ",isNum=" + isNum + ",order=" + order + ",ret=" + ret);
    return ret;
}
/**
 * 取0~n的随机整数
...
function tap(obj) {
    ...
    if (obj && !isUIAElementNil(obj)) {
        logDebug(msg + ",valid=" + obj.isValid() + ",可见=" + obj.isVisible());
         if (obj.isVisible()) {// obj.isValid() &&
            obj.tap();
            ret = true;
         }
    } else {
