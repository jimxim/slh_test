...
function test170048() {
    ...
    debugArray(alertMsgs);
    var alertMsg1 = getArray1(alertMsgs, -1);
    var ret1 = isAnd(isIn(alertMsg1, "保存成功"), !isIn(alertMsg, "本单余额计算有误"));
...
function test170080_170084() {
    ...
    var price = qr.data[0]["单价"];
    var zk = qr.data[0]["折扣"];
    var totalMoney = add(Math.round(price * num * zk), -9);
...
function test170121_170523() {
    ...
    tapFirstText();
    qr1 = getQRDet();
    var ret1 = isAnd(ret1, isEqual(Number(kc) - 9, qr1.data[0]["库存"]), isEqual(
            Number(stock1) - 8, qr1.data[1]["库存"]));
    tapReturn();
...
function test170209() {
    ...
    json = { "特殊货品" : { "抹零" : 18 } };
    editSalesBillSpecial(json);
    var ret3 = !isIn(alertMsg, "在对应款号的颜色组中不存在");
    tapReturn();
...
function test170215() {
    ...
    var ret1 = isAnd(isEqual(-2, qr.data[0]["数量"]),
            isIn(alertMsg1, "保存成功，是否打印"));
    ...
    tapFirstText();
    qr = getQRDet();
    var ret2 = isAnd(isEqual(je, getTextFieldValue(window, 2)), isEqual(
            "3035,jkk", qr.data[2]["货品"]), isEqual(-2, qr.data[2]["数量"]),
            isEqual("00000,抹零", qr.data[0]["货品"]),
            isEqual(-1, qr.data[0]["数量"]), isEqual("00001,打包费",
                    qr.data[1]["货品"]), isEqual(1, qr.data[1]["数量"]), isEqual(r,
...
function test170216() {
    ...
    tapFirstText();
    var qr = getQRDet();
    var ret1 = isAnd(isEqual(je, money), isEqual("3035,jkk", qr.data[2]["货品"]),
            isEqual(3, qr.data[2]["数量"]),
            isEqual("00002,免单", qr.data[1]["货品"]),
            isEqual(-1, qr.data[1]["数量"]), isEqual("00001,打包费",
                    qr.data[0]["货品"]), isEqual(1, qr.data[0]["数量"]), isEqual(r,
...
function test170559_170697() {
    ...
    // tapButtonAndAlert("挂 单", OK);
    // tapPrompt();
    // var ret6 = isIn(alertMsg, "本单不允许执行挂单操作");&& ret6+ ", ret6=" + ret6
...
function test170723() {
    ...
    var f4 = new TField("数量", TF, idx, 12);
    var fields = [ f4 ];
    setTFieldsValue(getScrollView(-1), fields);
    saveAndAlertOk();
    tapPrompt();
