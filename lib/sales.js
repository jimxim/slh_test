...
/**
 * 获取开单window界面中的值 需要添加accessibilityLabel
 * @returns
 */
function editSalesBillGetValue2() {
    // var tf = getTFieldsIndex(window);
    var tf = getValueFromCacheF1("getWindowTFieldsIndex");
    var arr = {};
    for ( var i in tf) {
        var index = tf[i];
        var v = getTextFieldValue(window, index);
        if (i == "null") {
            i = "总计";
        }
        arr[i] = v;
    }
    debugObject(arr, "开单界面值为");
    return arr;
}
