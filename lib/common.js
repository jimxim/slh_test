...
function isAqualOptime(expected, actual, allow) {
    ...
    var a11 = a1[1].split(":");
    var a21 = a2[1].split(":");
    ret = isAnd(ret, a11[0] == a21[0], isAqualNum(a11[1], a21[1]));
    logDebug("expected=" + expected + ",actual=" + actual + ",allow=" + allow
            + ",ret=" + ret);
    return ret;
}
...
function getPop(view1, i) {
    ...
    if (isUndefined(i) || i == 0) {
        ...
        for (var index = a1.length - 1; index >= 0; index--) {
            var e1 = a1[index];
            if (isUIAPopover(e1)) {
                total++;
...
function scrollLeft() {
    delay();
    target.dragFromToForDuration({ x : 600, y : 400 }, { x : 500, y : 400 }, 1);
}
/**
 * 向右滑动
 */
function scrollRight() {
    delay();
    target.dragFromToForDuration({ x : 500, y : 400 }, { x : 600, y : 400 }, 1);
}
/**
 * 下一页
 */
