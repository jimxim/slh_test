...
UIATarget.onAlert = function onAlert(alert) {
    ...
    logDebug("onAlert alertName=" + alertName + " alertMsg=" + alertMsg + " ret="+alertRet);
...
function goPage(i, qr) {
    ...
    if(total>1 && i<=total && i>0) {
        window.staticTexts()[curPageIndex].tapWithOptions({tapOffset:{x:0.08, y:0.55}});
...
function tapButtonAndAlert(i, alertButtonIndex) {
    ...
    try {
        ...
        logError("tapButtonAndAlertOk,i=" + i+",alertButtonIndex"+alertButtonIndex+",e"+e);
    }
    alertRet = false;
}
...
function setTFieldsValue(view1, fields) {
    ...
    for ( var i in fields) {
        var f = fields[i];
        logDebug("f.label=" + f.label + " type=" + f.type + " index=" + f.index
                + " value=" + f.value);
        switch (f.type) {
        case TF:
