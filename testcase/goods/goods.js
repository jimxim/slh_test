...
function testGoods002() {
    ...
    run("【货品管理】货品管理-货品查询，显示条码功能", "ts100058");
    run("【货品查询】查询条件“是否停用 ”需要默认为“否", "ts100124");
...
function ts100023() {
    var r = getTimestamp(8);
    var keys1 = { "款号" : "g" + r, "名称" : "货品" + r, "品牌" : "1010pp",
        "颜色" : "0,1", "尺码" : "0,1", "进货价" : "100" };
    var keys2 = { "颜色" : "花色,黑色", "尺码" : "S,M", "零批价" : "130", "打包价" : "120",
        "大客户价" : "160", "Vip价格" : "150" };
...
function ts100029() {
    var r = getTimestamp(8);
    var keys1 = { "款号" : "g" + r, "名称" : "货品" + r, "品牌" : "1010pp",
        "颜色" : "0,1", "尺码" : "0,1", "吊牌价" : "200", "产品折扣" : "0.85",
        "季节" : "夏季", "类别" : "登山服", "厂商" : "Adidas", "计量单位" : "双", "仓位" : "默认",
        "最小库存" : "0", "最大库存" : "1000", "经办人" : "000", "允许退货" : "是",
        "是否加工款" : "是", "加工价" : "150", "门店" : "常青店", "装箱数" : "12",
        "条码" : "abc123", "备注" : "123" };
 ...
 * @returns
 */
function ts100033Field(key1, key2) {
    tapMenu("货品管理", "新增货品+");
...
function testGoodsPriceChangeField(keys, expected) {
    ...
    tapButton(getScrollView(-1, 0), OK);// 0
    // var kb = app.keyboard();
    // if (kb.isVisible()) {
    // kb.typeString("slh000");
    // }
    ...
    if (isEqualsTexts1(texts, "吊牌价")) {
        ret = ret
                && isEqual("200", getTextFieldValue(getScrollView(), idx - 1));
    }
    ret = ret
            && isEqual("100", getTextFieldValue(getScrollView(), idx))
            && isAqualNum(expected,
                    getTextFieldValue(getScrollView(), idx + 1), 0.99)
            && isAqualNum(expected,
                    getTextFieldValue(getScrollView(), idx + 2), 0.99)
            && isAqualNum(expected,
                    getTextFieldValue(getScrollView(), idx + 3), 0.99)
            && isAqualNum(expected,
                    getTextFieldValue(getScrollView(), idx + 4), 0.99);
...
// 会修改所有的货品价格，不适合自动化跑 这里的零批价等文本框只是隐藏了，还是取得到
// function ts100051() {
// tapMenu("货品管理", "货品查询");
// var r = getRandomNum(201, 1000);
// query();
...
// // 统一加
// var keys = { "统一加减" : "200" };
// var ret = testGoodsPriceChangeField(keys, r + 200);
//
// // 统一乘 正值
// keys = { "统一乘" : "0.5" };
// ret = isAnd(ret, testGoodsPriceChangeField(keys, r * 0.5));
...
// 004店长登陆，需要后台勾选当前库存款号库存全局设置权限
function ts100100() {
    var qo, o, ret = true;
    qo = { "备注" : "门店库存是否允许跨门店查询" };
    o = { "新值" : "0", "数值" : [ "不允许门店相互查询库存", "in" ] };
    ret = isAnd(ret, setGlobalParam(qo, o));

    tapMenu("货品管理", "当前库存");
    query();
    tapTitle(getScrollView(), "仓库/门店");// 降序使中洲店的数据前移
    tapTitle(getScrollView(), "仓库/门店");
    var qr = getQR();
    var stock1 = qr.counts["库存"];
    var ret = isEqualQRData1ByTitle(qr, "仓库/门店", "常青店");
    ...
    tapTitle(getScrollView(), "仓库/门店");
    qr = getQR();
    ret = isAnd(ret, isEqualQRData1ByTitle(qr, "仓库/门店", "常青店"));
    ...
    ret = isAnd(ret, qr.data.length > 0, add(stock1, stock2) >= stock);
...
function addGoodsSimple() {
    ...
    case "no":
        ...
        break;
    case "yes":
        keys = { "款号" : code, "名称" : "货品" + r, "颜色" : "0,1", "尺码" : "0,1",
            "进货价" : "200" };
        break;
    default:
        break;
