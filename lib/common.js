...
function TField(label, type, index, value, p1, p2) {
	this.label = label; // 标题
...
function tapFirstText(view1, firstTitle) {
	...
	var i = getFirstIndexOfTextsByTitle(texts, firstTitle)
	tap(texts[i]);
}
// 保存时提示，点确定
...
function getQResultTitle(texts, title) {
	var titleIndexPre = 0;
	var titleIndex = 0;
	for (var i = 0; i < texts.length; i++) {
...
function setTextFieldSCValue(view1, e1) {
	var i = e1.index;
	var value = e1.value;
...
// 点击区域单选按钮
function setButtonAreaValue(view1, e1) {
	var btnIndex = e1.index;
	tapButton(view1, btnIndex);

	var value = e1.value;
	var tableViewIndex = 0;
	if(!isUndefined(e1.p1)) {
		tableViewIndex = e1.p1;
	}
	var view1 = getTableViews(getPop())[tableViewIndex];
	
	if(!isUndefined(e1.p2)) {
		var expand = e1.p2;
		var a1 = expand.split(",");
		for(var i in a1) {
			var v = a1[i];
			var cell = view1.cells()[v];
			cell.tapWithOptions({
				tapOffset : {
					x : 0.9,
					y : 0.5
	...
	var cell = view1.cells()[value];
	tap(cell);

	delay(); // 单选过后要加个延时，不加后续元素会无效
}
// 点击单选按钮
function setSingleChoiceValue(view1, e1) {
	var btnIndex = e1.index;
	tapButton(view1, btnIndex);
...
// 点击多选按钮
function setMultiChoiceValue(view1, e1) {
	var btnIndex = e1.index;
	tapButton(view1, btnIndex);

	var valueBtnIndexes = e1.value;
