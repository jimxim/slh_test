...
var TF = "tf";
var TF_AC = "tf-ac";
var TF_KB = "tf-kb";// 模拟键盘输入
var TF_SC = "tf-sc";
var TF_DT = "tf-dt";
var TF_TYPE = "tf-type";// 7.2731 类别控件
var TF_AREA = "tf-area";
...
var BTN_TYPE = "btn-type";// 7.2731 新增货品-类别-选择
...
/**
 * 图片选款界面 货品查询结果
 * @param code 款号 点击会显示大图
 * @param name 名称 同款号占同一个文本
 * @param det 单价/库存/订货数等 不同模块显示不同
 * @param days 上架天数 eg 14天，99+
 * @param addIdx 上架天数下标 点击会增加数量配合购物车使用 颜色尺码模式会弹表
 * @param subIdx days下标 点击会减去数量
 */
function PResult(code, name, det, days, addIdx, subIdx) {
    this.code = code;
    this.name = name;
    this.det = det;
    this.days = days;
...
function checkShowFields(view1, fields) {
    ...
    for ( var i in fields) {
        var f = fields[i];
        ret = ret && checkShowField(view1, f);
    }
    return ret;
}
function checkShowField(view, f) {
    var idx = f.index;
    var actual;
    var type2 = f.type.substr(0, 2);
    switch (type2) {
    case TF:
        actual = getTextFieldValue(view, idx);
        break;
    case TV:
        actual = getTextViewValue(view, idx);
        break;
    default:
 ...
 * @param name1 按钮名称
 * @param f1 函数 name(默认) or value
 * @param num 同界面相同名称按钮，如清除
 * @returns {Number}
 */
function getButtonIndex(view1, name1, num) {
    if (isUndefined(num)) {
        num = 1;
    }
    var index = -1, n = 0;
    ...
    logDebug("按钮名" + num + "=" + name1 + " index=" + index);
    return index;
}
/**
 * 获取按钮数组
...
function getQResult(pageInfoView, dataView, firstTitle) {
    ...
    var qrTitle = getQResultTitle(texts, firstTitle, firstTitle);
...
function getQResult2(dataView, firstTitle, lastTitle) {
    ...
    var ignoreNum = 0;// 忽略的标题数，如 图片
    var titles = new Array(), oTitle = {};
    for (var i = 0; i < titleTotal; i++) {
        var j = i - ignoreNum;
        var text = texts[titleIndex + i];
    ...
    titleTotal = titleTotal - ignoreNum;
    ...
    for (var i = 0; i < curPageTotal; i++) {
        ...
        for (var j = 0; j < titleTotal; j++) {
            var t = titles[j];
            var index = lastTitleIndex + 1 + i * titleTotal + j;
            if (index < texts.length) {
    ...
    // 小计 最后2个statisticText是分页信息
    var counts = {};
    var index = lastTitleIndex + curPageTotal * titleTotal + 1;// 可验证条目数是否正确
    for (index; index < texts.length - 2; index++) {
        var text = texts[index];
        var v1 = text.value();
        var x1 = getX(text);
        var t = oTitle[x1];
 ...
 * @param view1 默认getScrollView(-1)
 * @param offset 内容与其标题X轴的向左偏移量 默认0，一般小于10
 * @returns {QResult}
 */
function getQRtable1(view1, offset) {
    if (isUndefined(view1)) {
        view1 = getScrollView(-1);
    }
    var table1 = getLastTableView(view1);
    var group1 = table1.groups()[0];// 标题行
    ...
    var curPageTotal = total;// 全显示在一页中
    if (curPageTotal > PAGE_MAX_TOTAL) {
        curPageTotal = PAGE_MAX_TOTAL;// 一般最多只需要取15行数据
    }
    var data = new Array(curPageTotal);
    for (var i = 0; i < curPageTotal; i++) {
 ...
 * @returns {QResult}
 */
function getQRverify(texts, firstTitle, excludeTitleNum, titleTexts) {
    ...
    var qrTitle = getQResultTitle(titleTexts, firstTitle, firstTitle);
...
function getTableViewCells(view1, f, o) {
    ...
    return window.tableViews()[tableViewIndex].cells();
}
/**
 * 获取表格视图数组
 * @param view1 不输入默认window
 ...
 * 获取自定义Field对象集
 * @param fn1 定义函数
 * @param view textField所在界面
 * @param keys 关键字数组或对象
 * @param show 是否显示值
 * @returns
 */
function getTFields(fn1, view, keys, show) {
    if (isUndefined(keys)) {
        return {};
    }
    var ret = {}, tf = {};
    // 7.27起textField添加accessibilityLabel，改变获取方式
    if (ipadVer >= 7.27) {
        tf = getTFieldsIndex(view);// getValueFromCacheF1
    }
    for ( var i in keys) {
        var key = i;
        if (isArray(keys)) {
        ...
        if (ipadVer >= 7.27 && f.type.indexOf(TF) != -1) {
            var label = f.label;// needsearchfield
            var idx = tf[label];
            if (isDefined(idx)) {// 输入错误，会报服务端错误
                f.index = idx;
            }
        }
...
function isEqualQRData1ByTitle2(qr, title, expected) {
    ...
    for (var i = 0; i < qr.data.length; i++) {
        ...
        if (expected != v) {
            ret = false;
            logDebug("expected=" + expected + "  v=" + v + "  isEqual=" + ret);
            break;
        }
    }
    return ret;
...
function isEqualQRData1Object(qr, expected) {
    ...
    for (var i = 0; i < qr.data.length; i++) {
        var data1 = qr.data[i];
        ret = isEqualObject2(expected, data1);
        if (ret) {
            break;
        }
    }
 ...
 * @param username
 * @param password
 * @param p1 扩展属性，{"角色":"财务员","门店":"常青店","帐套":"autotest1"} 财务员需要选择门店
 * @returns
 */
function login(username, password, p1) {
    ...
    if (isDefined(account)) {
        msg = account + " " + msg;
        tapButton(window, account);// 先选帐套再输入角色密码
    }
    UIALogger.logStart(msg);

    tap(getTextField(window, 0)); // 为了在命令行下可顺利弹出自定义键盘而加

    if (role.indexOf("总经理") != -1) {
        password = "a" + password;// 总经理角色需要控制密码 统一加a
    }
    ...
    shop && tapButton(getScrollView(), shop);
 ...
 * @param view 默认window
 */
function query(qFields, tapClear, view) {
    if (isUndefined(view)) {
        view = window;
    }
...
function run(caseName, fTestcase) {
    ...
    try {
        ...
        // isInArray(alertMsgs, "错误代码")
        if (ok && (isInAlertMsgs("错误代码") || isInAlertMsgs("网络异常"))) {
            logInfo(alertMsgs);
            ok = false;
        }
        if (ok) {
...
function scrollNextPage() {
    delay(0.5);
    target.dragFromToForDuration({ x : 600, y : 400 }, { x : 600, y : 300 },
            0.5);
}
/**
 * 上一页
...
function scrollPrevPage() {
    delay(0.5);
    target.dragFromToForDuration({ x : 600, y : 300 }, { x : 600, y : 400 },
            0.5);
}
/**
 * 标题滑动至可见
 ...
 */
function tapFirstText(view1, firstTitle) {
    if (isUndefined(view1)) {
        view1 = getScrollView();
    }
    if (isUndefined(firstTitle)) {
    ...
    var texts = getStaticTexts(view1);
    var i = getFirstIndexOfTextsByTitle(texts, firstTitle, firstTitle);
...
function tapKeyboardHide() {
    ...
    if (isUIAElementNil(kb) || !kb.isVisible()) {
        ...
        tap(btn1);
        delay(0.5);// 键盘还未完全收回，界面元素获取错误 0914
    }
}
...
function tapStaticText(view1, i) {
    ...
    logDebug("tapStaticText view1=" + view1 + " i=" + i + " e1=" + e1);
    ...
    try {
        ...
        try {
            var e2 = getStaticText(view1, i + 2);
            if (e2 && !e2.isVisible()) {
                e2.scrollToVisible();
            }
        } catch (e) {
...
function tapTableViewCell(f) {
    var tableViewIndex = f.p1;
    var cellIndex = f.p2;
    var view = window, exclude = 1;
    if (cellIndex == "pop") {
        cellIndex = 0;
        view = getPop(window, -1);
        exclude = 0;
    }
    var tableViews = getTableViews(view);
    if (tableViewIndex < 0) {
        tableViewIndex = tableViews.length + tableViewIndex;
        while (tableViewIndex > exclude) {// 0是菜单，要排除掉
    ...
    logDebug("tableViews length=" + tableViews.length + " tableViewIndex="
            + tableViewIndex + ",cellIndex=" + cellIndex + ",ok=" + ok);
    tapKeyboardHide();// 收键盘，防止挡住后续元素
}
/**
 * 非alert提示
 * @param i 如果不输入，默认确定
