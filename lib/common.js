...
UIATarget.onAlert = function onAlert(alert) {
	...
	logDebug("alertName=" + alertName + " alertMsg=" + alertMsg + " ret=false");
...
//	target.pushTimeout(1);
	...
	if (isUIAElementNil(kb) || !kb.isVisible()) {
		logWarn("kb="+kb+" isVisible="+kb.isVisible());
	} else {
...
// json是否相等
function isEqualJson(expected, actual) {
	var ret = true;
		...
		for (var i = 0; i <= curPageTotal; i++) {
			...
			for (var j = 0; j < titleTotal; j++) {
				...
				if (index < texts.length) {
					...
					logWarn("index=" + index + " texts.length=" + texts.length);
				}
			}
			if (i < curPageTotal) {
				data[i] = data1;
...
function getTFields(fn1, keys, show) {
	...
	for (var i = 0; i < keys.length; i++) {
		...
		var f = eval(f1);
		logDebug("key=" + key + " f1=" + f1 + " val=" + f.value);
		ret[key] = f;
	}
...
function setTextFieldValueByKeyboard(tf, value, kbType) {
	// logDebug("tf.isVisible()="+tf.isVisible()+"
	// kb.isVisible()="+app.keyboard().isVisible());
	// debugElementTree(app.keyboard());
	if (!tf.isVisible()) {
		tapKeyboardReturn();
		// delay();
...
function setTFieldsValue(view1, fields) {
	...
	for ( var i in fields) {
		var f = fields[i];
		logDebug("f.label=" + f.label + " type=" + f.type + " index=" + f.index+ " value=" + f.value);
		switch (f.type) {
		case TF:
